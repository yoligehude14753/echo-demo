...
def _happy_facts() -> PermissionFacts:
    return _facts(
        ...
        CapabilityFact(
            "skill.use",
            {
                "identity": "echo/transcript",
                "version": "1.2.3",
                "provenance": "fact:F-ECHO-SKILL-1",
            },
        ),
    )
...
def test_compile_and_decide_happy_path_produces_immutable_grant() -> None:
    ...
    assert decide(
        result, "path.read", {"platform": "posix", "path": "/workspace/project/src/app.py"}, now=NOW
    ).allowed
    assert decide(
        result,
        "command.execute",
...
def test_path_root_requires_explicit_platform_and_blocks_prefix_escape() -> None:
    ...
    assert not decide(
        result, "path.read", {"platform": "posix", "path": "/workspace/project-evil/file"}, now=NOW
    ).allowed
    assert decide(
        result, "path.read", {"platform": "posix", "path": "/workspace/project/ok"}, now=NOW
    ).allowed


def test_path_normalization_is_lexical_and_rejects_host_dependent_forms() -> None:
    assert (
        normalize_path_root("/workspace/./project/../project", platform="posix").root
        == "/workspace/project"
    )
    assert (
        normalize_path_root(r"C:\\Echo Desk\\project\\..\\project", platform="windows").root
        == r"C:\Echo Desk\project"
    )
...
def test_command_authority_is_exact_argv_and_cwd(
    command_argv: tuple[str, ...], reason: ReasonCode
) -> None:
    facts = _facts(
        CapabilityFact(
            "command.execute",
    ...
    assert (
        ...
        is ReasonCode.COMMAND_NOT_AUTHORIZED
    )
...
def test_command_shell_text_and_env_assignment_are_fail_closed() -> None:
    shell_text = compile_grant(
        _facts(
            CapabilityFact(
                "command.execute", {"platform": "posix", "argv": "git status", "cwd": "/workspace"}
            )
        ),
        task_id="task",
        now=NOW,
    ...
    authority = normalize_command_scope(
        ("git",), cwd="/workspace", env_names=("LANG",), platform="posix"
    )
    assert authority.env_names == ("LANG",)


def test_network_matrix_blocks_ssrf_and_unverified_or_redirected_targets() -> None:
    private = compile_grant(
        _facts(
            CapabilityFact("network.connect", {"scheme": "http", "host": "127.0.0.1", "port": 80})
        ),
        task_id="task",
        now=NOW,
    )
    ...
    unverified = compile_grant(
        _facts(
            CapabilityFact(
                "network.connect", {"scheme": "https", "host": "api.example", "port": 443}
            )
        ),
        task_id="task",
        now=NOW,
...
def test_unknown_capability_conflict_and_revision_freshness_never_allow() -> None:
    unknown = compile_grant(
        _facts(CapabilityFact("filesystem.delete", {"root": "/workspace", "platform": "posix"})),
        task_id="task",
        now=NOW,
    )
    ...
    conflict = compile_grant(
        _facts(
            ...
            CapabilityFact(
                "path.read", {"platform": "posix", "root": "/workspace/project"}, "deny"
            ),
        ),
        task_id="task",
        now=NOW,
    ...
    stale = compile_grant(
        _facts(
            CapabilityFact("path.read", {"platform": "posix", "root": "/workspace"}), revision=0
        ),
        task_id="task",
        now=NOW,
    )
    assert stale.reason_code is ReasonCode.STALE_REVISION
    marked_stale = compile_grant(
        PermissionFacts(
            7,
            (CapabilityFact("path.read", {"platform": "posix", "root": "/workspace"}),),
            stale=True,
        ),
        task_id="task",
        now=NOW,
    ...
    expired = compile_grant(
        _facts(
            CapabilityFact("path.read", {"platform": "posix", "root": "/workspace"}),
            expires_at="2026-07-15T08:59:00Z",
        ),
        task_id="task",
        now=NOW,
    )
...
def test_skill_identity_and_version_are_exact_and_ranges_are_ambiguous() -> None:
    result = compile_grant(
        _facts(
            CapabilityFact(
                "skill.use",
                {"identity": "echo/transcript", "version": "1.2.3", "provenance": "fact:F-SKILL-1"},
            )
        ),
        task_id="task",
        now=NOW,
    ...
    assert (
        decide(
            result,
            "skill.use",
            {"identity": "echo/transcript", "version": "1.2.4", "provenance": "fact:F-SKILL-1"},
            now=NOW,
        ).reason_code
        is ReasonCode.SKILL_NOT_AUTHORIZED
    )
    assert (
        decide(
            result,
            "skill.use",
            {"identity": "echo/transcript", "version": "1.2.3", "provenance": "fact:F-SKILL-2"},
            now=NOW,
        ).reason_code
        is ReasonCode.SKILL_NOT_AUTHORIZED
    )
    with pytest.raises(ValueError):
        normalize_skill_scope(
            {"identity": "echo/transcript", "version": "latest", "provenance": "fact:F-SKILL-1"}
        )
    with pytest.raises(ValueError):
