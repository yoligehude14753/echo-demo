...
@pytest.mark.asyncio
@pytest.mark.unit
async def test_transcribe_accepts_wav_without_double_wrapping(settings: Settings) -> None:
    stt = FireRedSTT(settings)
    pcm = b"\x01\x00\x02\x00" * 4000
    wav = pcm_to_wav(pcm, sample_rate=16_000)
    fake = _mock_async_client_post({"text": "你好世界"})

    with patch("app.adapters.stt.firered.httpx.AsyncClient", return_value=fake):
        segs = await stt.transcribe(wav, sample_rate=16_000)

    assert segs[0].text == "你好世界"
    posted_wav = fake.post.await_args.kwargs["files"]["file"][1]
    with wave.open(io.BytesIO(posted_wav), "rb") as wf:
