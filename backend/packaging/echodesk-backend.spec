...
def is_legacy_app_module(name):
    return any(name == prefix or name.startswith(f"{prefix}.") for prefix in LEGACY_APP_MODULE_PREFIXES)


hiddenimports = [
    name for name in collect_submodules("app") if not is_legacy_app_module(name)
]
hiddenimports.extend(collect_submodules("yoli_llm"))
datas = collect_data_files(
    "app",
    excludes=["adapters/skill/assets/ppt_ib_deck"],
)
datas.extend(collect_data_files("yoli_llm", include_py_files=True))
datas.append((str(BACKEND_ROOT / "app" / "model_gateway_requirements.json"), "app"))
binaries = []
...
a = Analysis(
    ...
    pathex=[str(BACKEND_ROOT), str(SHARED_LLM_ROOT)],
    binaries=binaries,
    datas=datas,
