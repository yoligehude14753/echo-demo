...
def _injected_runtime_endpoint() -> tuple[str, int]:
    raw = os.getenv("ECHODESK_BASE_URL", "").strip().rstrip("/")
    parsed = urlsplit(raw)
    if (
        ...
        raise RuntimeError("ECHODESK_BASE_URL must be an absolute HTTP(S) origin")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    endpoint_host, endpoint_port = _injected_runtime_endpoint()
    server_args.host = server_args.host or endpoint_host
    server_args.port = server_args.port or endpoint_port
    _ensure_source_backend_path()
    import uvicorn
