...
class SessionStore:
    ...
    async def _device_has_sync_references_tx(
        self,
        conn: aiosqlite.Connection,
        *,
        tenant_id: str,
        ...
        """Keep a device identity while Hub still references it.

        Expired public credentials may be collected, but a Hub pairing or a
        synchronized device is durable continuity state.  Deleting the parent
        device first violates those foreign keys and turns the next enrollment
        into a server error.  Retaining the otherwise empty device/user scope is
        safe; its expired bearer and enrollment are still removed above.
        """

        checks = (
            ("sync_devices", "device_id"),
            ("device_pairings", "source_device_id"),
        )
        for table, device_column in checks:
            cur = await conn.execute(
                f"""SELECT 1 FROM {table}
                    WHERE tenant_id = ? AND owner_id = ? AND {device_column} = ?
                    LIMIT 1""",
    ...
    async def _cleanup_orphaned_enrollments_tx(
        ...
        *,
        ...
        for row in rows:
            ...
            if await self._device_has_sync_references_tx(
                conn,
                tenant_id=tenant_id,
                user_id=user_id,
                device_id=device_id,
