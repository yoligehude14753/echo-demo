...
class MeetingState:
    ...
    async def _settle_capture_requests(self, meeting_id: str) -> None:
        """Drain accepted ASR requests before the pipeline closes the append gate."""

        settler = getattr(self._pipeline, "settle_capture_requests", None)
        if callable(settler):
            await settler(meeting_id)
    ...
    def start_watchdog(self, *, interval_s: float = 5.0) -> None:
        ...
        新 chunk 触发检查。watchdog 只推进 auto meeting 的 silence/max
        检查。用户显式开始的 manual meeting 以按钮边界为权威，不能由等待、
        静音或运行时长替用户结束。
        """
    ...
    async def _hydrate_from_repo(self) -> None:
        ...
        2026-08 修订：同样清理历史桌面端遗留的 m-local-*；普通手动会议
        不再按进程重启、静音或年龄自动结束。它们是用户显式建立的边界，重启后
        应恢复为 current，直到用户显式点击结束。
        """
        ...
        if uses_system_limit and age_s > self._max_meeting_duration_s:
            try:
    ...
    async def recover_stuck_minutes(self) -> int:
        ...
        触发条件（任一）：
        - ``state="ended"`` 且 ``minutes_status IS NULL``（旧版本崩溃路径）
        - ``state="ended"`` 且 ``minutes_status="generating"``（进程中断路径）

        ``generation_failed`` 是用户可见的终态，必须保留失败历史并由显式
        ``POST /meetings/{id}/finalize`` 手动重试，启动恢复绝不自动重跑。
        ...
        stuck = [
            m
            for m in meetings
            if m.minutes_status in {None, "generating"}
            and not m.minutes_json
    ...
    async def manual_start(self, *, title: str | None = None) -> CurrentMeeting:
        ...
        async with self._lock:
            ...
            record = await self._pipeline.start_meeting(
                mid,
                title=title,
                auto_started=False,
                state_event_reason="user_clicked",
            )
            authoritative = (
                record
    ...
    async def end_without_finalize(
        ...
        *,
        ...
        await self.hydrate()
        ended_at = datetime.now(UTC)
        await self._settle_capture_requests(meeting_id)
        async with self._lock:
    ...
    async def _end_current(
        ...
        *,
        ...
        # Capture claims are bound to the meeting before ASR starts. Keep the
        # meeting active while they drain; only then does end_meeting establish
        # the durable append fence.
        await self._settle_capture_requests(meeting_id)
        async with self._lock:
            cur = self._current
    ...
    async def observe_chunk(
        ...
        *,
        ...
        for ev in events:
            if ev.kind == "start":
                await self._apply_auto_start(
                    ev.meeting_id,
                    reason=ev.reason,
                    started_at=chunk_started_at or now,
                    observed_at=now,
                )
            elif ev.kind == "end":
                await self._apply_auto_end(
                    ev.meeting_id,
                    reason=ev.reason,
                    ended_at=now,
                )
    ...
    async def note_valid_speech(
        ...
        *,
        ...
        """显式 meeting_id 上传路径续期匹配会议的有效语音心跳。"""
        ...
        if cur is not None and cur.meeting_id == meeting_id:
            ...
            self._last_valid_speech_at = now_aware
            if cur.started_by == "auto":
                self._adopt_current_auto_meeting(now_aware)
                self._detector.note_voice_activity(now=now_aware)

    async def note_acoustic_activity(
        self,
        *,
        now: datetime,
        meeting_id: str | None = None,
    ) -> None:
        """在 ASR 完成前用强声学证据续命当前自动会议。

        只更新已经存在的 auto meeting；不会创建会议，也不会改变 manual meeting
        的显式边界。显式 ``meeting_id`` 必须匹配当前 auto meeting，避免陈旧尾片
        续期另一个会议。这样远端 ASR 排队或短文本被幻觉门丢弃时，watchdog 仍以
        真实采集活动为准，而不是把模型处理延迟误判为静默。
        """
        ...
        async with self._lock:
            ...
            if cur is None or cur.started_by != "auto":
                return
            if meeting_id is not None and cur.meeting_id != meeting_id:
                return
            self._last_valid_speech_at = now_aware
            self._adopt_current_auto_meeting(now_aware)
            self._detector.note_voice_activity(now=now_aware)
    ...
    async def _check_lifecycle(  # noqa: PLR0911
        ...
        """推进自动会议的 hard-limit / silence；命中时返回 True。"""
        ...
        # Manual boundaries are explicit user state.  Valid-speech heartbeats
        # remain useful diagnostics, but no process-local clock may turn them
        # into an implicit end action.
        return False
    ...
    async def _meeting_exceeded_max_duration(self, now: datetime) -> bool:
        ...
        silence/max 检查。显式 manual meeting 不经过本分支。
        """
        cur = self._current
        if cur is None:
            return False
        ...
        max_duration_s = self._max_meeting_duration_s
    ...
    async def _apply_auto_start(
        ...
        *,
        ...
        boundary_at = started_at or datetime.now(UTC)
        if boundary_at.tzinfo is None:
            boundary_at = boundary_at.replace(tzinfo=UTC)
        activity_at = observed_at or boundary_at
        ...
        async with self._lock:
            if self._current is not None:
                ...
                return
            auto_starter = getattr(
                self._pipeline,
                "start_auto_meeting_with_backfill",
                None,
            )
            if callable(auto_starter):
                record, backfilled = await auto_starter(
                    meeting_id,
                    detected_at=activity_at,
                    fallback_started_at=boundary_at,
                    state_event_reason=reason,
                )
            else:
                record = await self._pipeline.start_meeting(
                    meeting_id,
            ...
            self._last_valid_speech_at = boundary_at
            current = self._current
        if current.started_by == "auto":
            self._detector.adopt_active(
                current.meeting_id,
            ...
            await self._publish(
                ...
                current.meeting_id,
                {"reason": reason, "backfilled_segments": backfilled},
            )
        else:
