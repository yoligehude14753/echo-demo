"""LLM Search/Web 取证后回答，并约束引用真实性。

本地搜索由 Echo AI 主模型直接阅读文档目录与原文完成，不使用 BM25 或向量
排名。联网路径负责获取网页候选，主模型统一判断、汇总并生成带原始引用的答案。
"""
...
from app.use_cases.llm_search import search_local_evidence
...
_GROUNDED_ANSWER_SYSTEM_PROMPT = f"""你是 EchoDesk，会议与办公场景的数字分身。
只能使用本次提供的事实材料回答，不得用材料外的知识补齐事实。

{ANSWER_STYLE_CONTRACT}
事实与引用规则：
- 先综合材料形成自然、直接的答案，不要按 Memory、本地文档、Web 分来源汇报。
- 每个事实句或事实段末尾附上直接支持它的精确引用。
- 本地引用只能原样使用 [doc:doc_id-chunk_id]，联网引用只能原样使用 [web:url]，记忆引用只能原样使用 [memory:candidate_id]；不得编造或改写引用 ID。
- 有部分材料时，只回答材料确实覆盖的部分，静默省略其余部分。
- 一个引用不能支撑与其内容无关的数字、事实或结论；没有直接材料支持的内容不要输出。
- 关联记忆与当前问题直接匹配时可以优先使用，但不得让主题无关的任何材料覆盖它。
- 不要说明 Memory、本地文档或 Web 哪一类没有命中、材料无关或无法核对；有可用事实时
  直接回答可确认的内容，静默忽略无关来源。
"""

_ANSWER_INPUT_TEMPLATE = """<facts>
<memory>
{memory_block}
</memory>
<local_documents>
...
_ZERO_EVIDENCE_ANSWER = "我暂时无法确认这个问题。"
...
def _contains_process_narration(text: str) -> bool:
    normalized = text.lower().replace(" ", "")
    return any(marker in normalized for marker in _PROCESS_NARRATION_MARKERS)


def _asks_for_search_meta(question: str) -> bool:
    normalized = question.lower().replace(" ", "")
    return any(marker in normalized for marker in _SEARCH_META_MARKERS)
...
def _is_structural_line(stripped: str) -> bool:
    ...
    if stripped.startswith("#"):
        ...
        return heading not in _GENERIC_HEADINGS
...
class _EvidenceGuard:
    ...
    def process_line(self, line: str) -> str | None:  # noqa: PLR0911
        ...
        if not self.allow_search_meta and _contains_process_narration(stripped):
            return None
...
async def retrieve_and_answer(  # noqa: PLR0915
    *,
    ...
    if answer_sources is None:
        ...
        source_set = set(answer_sources)
        wants_local = "local_documents" in source_set
        wants_web = "web" in source_set
        cls = "either" if wants_local and wants_web else "rag" if wants_local else "web" if wants_web else "memory"
        logger.info(
            "latency stage=route source=intent_plan label=%s sources=%s elapsed_ms=0.0",
            cls,
    ...
    async def _retrieve_rag() -> list[RagChunk]:
        ...
        if cls not in {"rag", "either"}:
            ...
            return []
        try:
            chunks = await search_local_evidence(
                llm=main_llm,
                model=main_model,
                rag=rag,
                question=question,
            ...
            logger.info(
                "latency stage=llm_search status=ok count=%d elapsed_ms=%.1f",
                len(chunks),
            ...
            logger.error(
                "latency stage=llm_search status=failed error_type=%s elapsed_ms=%.1f",
                type(exc).__name__,
                (time.perf_counter() - started) * 1000,
            )
            raise
    ...
    async def _retrieve_web() -> list[WebHit]:
        started = time.perf_counter()
        if cls not in {"web", "either"} or web_top_n <= 0:
    ...
    prompt = _ANSWER_INPUT_TEMPLATE.format(
    ...
    async def _gen() -> AsyncIterator[str]:  # noqa: PLR0912, PLR0915
        ...
        guard = _EvidenceGuard(
            ...
            allow_search_meta=_asks_for_search_meta(question),
        )
        messages = [
            ChatMessage(role="system", content=_GROUNDED_ANSWER_SYSTEM_PROMPT),
