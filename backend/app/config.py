...
P1.2（独立产品 Phase 1）：配置三层化
  ...
  `ECHODESK_BASE_URL` 例外：它只由 lifecycle owner 运行时注入，不能由用户配置
  或 .env 复活；详见 app/config_io.py。
"""
...
_MODEL_GATEWAY_DEFAULT_BASE_URL = "https://agent.yoliyoli.uk/v1"
_MODEL_GATEWAY_DEFAULT_CHAT_MODEL = "qwen3.5-35b-a3b"


def _normalized_gateway_secret(value: str | None) -> str:
    normalized = (value or "").strip()
    if not normalized or any(
        char.isspace() or ord(char) < 32 or ord(char) == 127 for char in normalized
    ):
        raise RuntimeError("invalid secret format")
    return normalized


def _read_gateway_key_file(path: Path) -> str:
    """Read only the owner-private regular files accepted by the gateway contract."""
    ...
    try:
        ...
        with os.fdopen(descriptor, "r", encoding="utf-8") as handle:
            ...
            if (
                ...
                raise ValueError
            return _normalized_gateway_secret(handle.read())
...
class Settings(BaseSettings):
    ...
    def settings_customise_sources(
        ...
        `ECHODESK_BASE_URL` 是 lifecycle owner 的运行时注入，不允许从
        user.json 或 .env 恢复旧值。
        """
        return (
            init_settings,
            env_settings,
            JsonConfigSource(settings_cls, excluded_fields=RUNTIME_ONLY_FIELDS),
            ExcludingSettingsSource(dotenv_settings, excluded_fields=RUNTIME_ONLY_FIELDS),
            file_secret_settings,
        )

    # ── Server ────────────────────────────────────────────────────
    # 端点只能由 lifecycle owner 注入；禁止为本地端口保留源码 fallback。
    echodesk_base_url: str = Field(
        default="",
        validation_alias=AliasChoices("ECHODESK_BASE_URL", "echodesk_base_url"),
    )
    log_level: str = "INFO"
    ...
    @field_validator("echodesk_base_url")
    @classmethod
    def _validate_runtime_endpoint(cls, value: str) -> str:
        normalized = value.strip().rstrip("/")
        if not normalized:
            raise ValueError("ECHODESK_BASE_URL must be injected by the backend lifecycle owner")
        ...
        if (
            ...
            raise ValueError("ECHODESK_BASE_URL must be an absolute HTTP(S) origin")
    ...
    @property
    def port(self) -> int:
        parsed = urlsplit(self.echodesk_base_url)
        return parsed.port or (443 if parsed.scheme == "https" else 80)

    @property
    def public_http_url(self) -> str:
        return self.echodesk_base_url

    @property
    def public_ws_url(self) -> str:
        parsed = urlsplit(self.echodesk_base_url)
        scheme = "wss" if parsed.scheme == "https" else "ws"
        return f"{scheme}://{parsed.netloc}/ws/echo"

    # ── Unified model gateway ─────────────────────────────────────
    # EchoDesk 的主 LLM 与 ASR 只注册到这一处网关。旧 provider 字段仍保留为
    # 读取兼容，不再参与这两条核心调用的 endpoint 或 credential 决策。
    model_gateway_base_url: str = Field(
        default=_MODEL_GATEWAY_DEFAULT_BASE_URL,
        validation_alias=AliasChoices(
            "model_gateway_base_url",
            "MODEL_GATEWAY_BASE_URL",
        ),
    )
    model_gateway_api_key: str | None = Field(
        default=None,
        repr=False,
        validation_alias=AliasChoices(
    ...
    model_gateway_api_key_file: str | None = Field(
        default=None,
        repr=False,
        validation_alias=AliasChoices(
            "model_gateway_api_key_file",
            "MODEL_GATEWAY_API_KEY_FILE",
        ),
    )
    model_gateway_service_name: str = Field(
        default="echodesk",
        validation_alias=AliasChoices(
    ...
    # 网关目录发现与每次模型请求共享的默认 deadline。各 adapter 可按调用覆盖，
    # 但不得退回到 transport 的无感知默认值。
    model_gateway_timeout_s: float = Field(
        default=45.0,
        gt=0.0,
        le=600.0,
        validation_alias=AliasChoices(
            "model_gateway_timeout_s",
            "MODEL_GATEWAY_TIMEOUT_S",
        ),
    )
    ...
    @classmethod
    def _canonicalize_model_gateway_base_url(cls, _value: object) -> str:
        return _MODEL_GATEWAY_DEFAULT_BASE_URL

    @field_validator("model_gateway_service_name", mode="before")
    @classmethod
    def _canonicalize_model_gateway_service_name(cls, _value: object) -> str:
        return "echodesk"

    @property
    def resolved_model_gateway_api_key(self) -> str:
        """Compatibility accessor; resolution is owned by the gateway factory."""
        from app.adapters.llm.model_gateway_factory import resolve_model_gateway_credential

        return resolve_model_gateway_credential(
            base_url=self.model_gateway_base_url,
            service_name=self.model_gateway_service_name,
            configured_key=self.model_gateway_api_key,
            configured_file=self.model_gateway_api_key_file,
        )

    # ── LLM 主通道（OpenAI-compatible unified gateway） ───────────
    llm_main_provider: str = "model_gateway"
    model_gateway_chat_model: str = Field(
        default=_MODEL_GATEWAY_DEFAULT_CHAT_MODEL,
        validation_alias=AliasChoices(
            "model_gateway_chat_model",
            "MODEL_GATEWAY_CHAT_MODEL",
        ),
    )
    # Legacy read compatibility. The canonical field above overwrites this value.
    ...
    # Compatibility input only. Runtime/UI identity is derived from llm_main_model.
    llm_main_display_name: str = "Qwen3.5"
    llm_main_base_url: str = _MODEL_GATEWAY_DEFAULT_BASE_URL
    ...
    llm_main_max_tokens: int = Field(default=4_096, ge=1, le=16_384)
    # 默认不静态指定纪要输出预算：由当前凭证的 fresh capability profile
    # 注入 output_limit，再按本次 prompt 与 context_limit 动态收紧。部署方仍可
    # 显式设置该值作为更小的业务预算，但不能把它扩大到 profile 边界之外。
    minutes_max_tokens: int | None = Field(default=None, ge=1, le=200_000)
    # A finalize owns this bounded retry budget. Each attempt re-resolves the
    # same-credential fresh capability profile; this is not a model fallback.
    minutes_llm_max_attempts: int = Field(default=3, ge=1, le=4)
    minutes_llm_retry_base_delay_s: float = Field(default=0.25, gt=0.0, le=5.0)
    minutes_llm_retry_max_delay_s: float = Field(default=2.0, gt=0.0, le=10.0)

    @property
    def resolved_llm_main_api_key(self) -> str:
        """Compatibility accessor for callers not yet using the gateway name."""

        return self.resolved_model_gateway_api_key

    # ── LLM 快速通道 ────────────────────────────────────────────────
    # 默认跟随 Yunwu 主通道，避免私有 fast LLM 未启动时影响源码安装；
    # 私有部署可在设置页改为自己的 vLLM / OpenAI-compatible 端点。
    llm_fast_provider: str = "model_gateway"
    llm_fast_model: str = Field(
        default=_MODEL_GATEWAY_DEFAULT_CHAT_MODEL,
        validation_alias=AliasChoices("llm_fast_model", "LLM_FAST_MODEL"),
    )
    llm_fast_base_url: str = _MODEL_GATEWAY_DEFAULT_BASE_URL
    # Compatibility input only. Runtime/UI identity is derived from llm_main_model.
    llm_fast_display_name: str = "Qwen3.5"
    ...
    @field_validator("llm_main_provider", "llm_fast_provider", mode="before")
    @classmethod
    def _canonicalize_llm_providers(cls, _value: object) -> str:
        return "model_gateway"

    @model_validator(mode="after")
    def _bind_llm_model_identity(self) -> Settings:
        """One gateway setting owns main, fast and compatibility labels."""

        canonical = self.model_gateway_chat_model.strip()
        self.model_gateway_chat_model = canonical
        self.llm_main_model = canonical
        self.llm_fast_model = canonical
        self.llm_main_display_name = canonical
        self.llm_fast_display_name = canonical
        return self

    @field_validator("llm_main_base_url", "llm_fast_base_url", mode="before")
    @classmethod
    def _canonicalize_llm_endpoints(cls, _value: object) -> str:
        return _MODEL_GATEWAY_DEFAULT_BASE_URL
    ...
    # ── STT ───────────────────────────────────────────────────────
    # Qwen3-ASR 只允许通过 EchoDesk owner 的 Model Gateway 能力合同调用。
    # 不再保留 provider、endpoint、credential 或本地模型 fallback 选择器。
    stt_model: str = Field(
    ...
    @classmethod
    def _canonicalize_stt_model(cls, _value: object) -> str:
    ...
    asr_model_gateway_concurrency: int = Field(default=1, ge=1, le=64)
    ...
    @model_validator(mode="after")
    def _apply_asr_runtime_defaults(self) -> Settings:
        # bundled local capture 的四条上传分片需要同样受限的四个网关
        # HTTP 槽位；全局八槽位允许有限跨 scope 并发，32 项 backlog 约为
        # 四个 scope 的短暂恢复窗口。public/cloud 保持默认值，必须显式放宽。
        if not self.public_demo_mode:
            local_defaults: dict[str, object] = {
                "asr_model_gateway_concurrency": 4,
                "asr_scheduler_max_concurrency": 8,
                "asr_scheduler_queue_size": 32,
                "asr_scope_max_concurrency": 4,
            }
            for field_name, value in local_defaults.items():
                if field_name not in self.model_fields_set:
                    setattr(self, field_name, value)
        return self

    # ── STT 后处理：LLM 补标点 + 分段 ──────────────────────────────
    # 用户痛点（2026-05-28）：ASR 转录可能输出一气呵成的长句，
    ...
    @classmethod
    def _canonicalize_tts_provider(cls, _value: object) -> str:
        return "qwen3_tts"

    @field_validator("tts_qwen3_url", mode="before")
    @classmethod
    def _canonicalize_tts_endpoint(cls, _value: object) -> str:
        return _MODEL_GATEWAY_DEFAULT_BASE_URL

    @field_validator("tts_model", mode="before")
    @classmethod
    def _canonicalize_tts_model(cls, _value: object) -> str:
        return "qwen3-tts"
    ...
    # 自动开始与会议续命使用比“允许转写入库”更严格的音频证据。有效时长既
    # 要满足累计活跃帧，也要形成同等长度的连续 voiced 段；零散噪声不能仅凭
    # 一段 ASR 文本触发或续命会议。
    ...
    # 自动会议的判定发生在若干有效片段之后；把判定前的 ambient 规范行回填到
    # meeting_segments，确保检测窗口与纪要窗口一致。只回填当前设备、当前 owner。
    automeet_backfill_window_s: float = Field(default=90.0, ge=0.0, le=600.0)
    # 静默 X 秒 → 自动 end（含 finalize 纪要）
    automeet_silence_timeout_s: float = 90.0
    # 自动结束后多久内不再触发新会议（防抖）
    automeet_cooldown_s: float = 15.0
    ...
    # Stop waits for already accepted formal capture requests before closing
    # meeting_segments. The bound prevents a crashed uploader from blocking
    # the state machine forever.
    meeting_capture_settle_timeout_s: float = Field(default=45.0, gt=0.0, le=300.0)
    ...
    # Bound one principal's durable original-text catalog and parsed chunk set
    # so a valid multi-file workload cannot exhaust the server. Relevance is
    # decided only by Qwen LLM Search, never by this storage layer.
    ...
    # Failed generated-code runs retain owner-private script/stdout/stderr evidence here.
    skill_executor_diagnostic_dir: Path = Field(
        default=Path("~/.echodesk/skill_diagnostics").expanduser()
    )
    skill_executor_timeout_s: int = 300
    skill_executor_max_tokens: int = 4_096
    ...
    # ── DB ────────────────────────────────────────────────────────
    # ``ECHO_USER_DIR`` 是本地用户数据根，也是源码/验收隔离的唯一根。
    # 默认值必须从它派生；否则只覆盖配置/数据库而把音频和产物误写进真实
    # ``~/.echodesk/storage``。未设置环境变量时 user_config_dir() 仍等于
    # ``~/.echodesk``，因此已安装用户的默认路径不变。
    db_path: Path = Field(default_factory=lambda: user_config_dir() / "echodesk.db")
    storage_dir: Path = Field(default_factory=lambda: user_config_dir() / "storage")
    ...
    # 不再只覆盖 multipart 上传。
    # Capture clients use at most four concurrent partitions. Keep independent
    # body admission comfortably above that cadence while retaining a strict
    # process-wide bound; body leases are released before ASR/runtime work.
    upload_global_concurrent_requests: int = Field(default=32, ge=1)
    ...
    def allowed_origins_list(self) -> list[str]:
        ...
        return list(
            dict.fromkeys(
                (*configured, self.echodesk_base_url, OFFICIAL_ELECTRON_ORIGIN)
            )
        )
