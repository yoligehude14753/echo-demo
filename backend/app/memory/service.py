...
from app.ports.llm import LLMPort, LLMPriority
...
class MemoryService:
    ...
    async def recall(
        ...
        *,
        ...
        matches, resolved_model = await self._associate(
            query,
            shortlist,
            result_limit,
        ...
        return RecallResult(
            query=query,
            matches=matches,
            used_small_model=resolved_model is not None,
    ...
    async def _associate(
        ...
        *,
    ...
    ) -> tuple[list[RecallMatch], str | None]:
        if not candidates:
            return [], None
        ...
        try:
            ...
            return (
                sorted(combined, key=lambda item: item.score, reverse=True)[:limit],
                response.model,
            )
        except Exception as error:
            logger.warning("memory association fell back: %s", _bounded_error(error))
            return self._fallback_matches(query, candidates, limit), None
    ...
    async def extract_text(
        ...
        *,
        ...
        started = perf_counter()
        try:
            raw_candidates = await self._extract_candidates(
                clean_text,
                existing,
                llm_priority=llm_priority,
            )
