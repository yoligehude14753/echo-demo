...
class PathVerifier:
    ...
    def _check_entry(cls, path: str, *, allow_missing: bool = False) -> os.stat_result | None:
        try:
            ...
            if allow_missing:
                return None
            raise PathHostError(DenyCode.TOOL_PATH_AMBIGUOUS, "path does not exist")
        except OSError as exc:
            raise PathHostError(DenyCode.TOOL_PATH_AMBIGUOUS, "path identity cannot be inspected") from exc
        if stat.S_ISLNK(info.st_mode):
            raise PathHostError(DenyCode.TOOL_PATH_AMBIGUOUS, "symlink path component is not allowed")
        if cls._is_reparse(info):
            raise PathHostError(DenyCode.TOOL_PATH_AMBIGUOUS, "reparse path component is not allowed")
        return info
    ...
    def _check_components(self, candidate: str, *, allow_missing: bool) -> os.stat_result | None:
        ...
        try:
            ...
            raise PathHostError(DenyCode.TOOL_PATH_OUTSIDE_WORKSPACE, "path has a different anchor") from exc
        ...
        for index, part in enumerate(parts):
            if part in {"", os.curdir, os.pardir}:
                raise PathHostError(DenyCode.TOOL_PATH_AMBIGUOUS, "path contains ambiguous components")
            current = os.path.join(current, part)
            info = self._check_entry(current, allow_missing=allow_missing and index == len(parts) - 1)
            if info is None:
                return None
    ...
    def verify(self, path: str | os.PathLike[str], *, allow_missing: bool = False) -> VerifiedPath:
        ...
        try:
            if os.path.commonpath((self.root_path, os.path.realpath(candidate))) != self.root_path:
                raise PathHostError(DenyCode.TOOL_PATH_IDENTITY_CHANGED, "real path escaped workspace")
        except ValueError as exc:
            raise PathHostError(DenyCode.TOOL_PATH_IDENTITY_CHANGED, "real path has a different anchor") from exc
        return VerifiedPath(
            path=candidate,
            root_id=self.root.root_id,
            root_identity=actual_root_identity,
            target_identity=(
                f"{target_info.st_dev:x}:{target_info.st_ino:x}" if target_info is not None else None
            ),
