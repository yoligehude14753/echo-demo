...
from app.adapters.rag import TextCatalogRag
...
from app.schemas.intent import AnswerSource
...
_RAG_RETRIEVAL_HEARTBEAT_S = 10.0


_rag_singleton: TextCatalogRag | None = None
...
def get_rag(settings: Settings = Depends(get_settings)) -> RagPort:
    ...
    if _rag_singleton is None:
        _rag_singleton = TextCatalogRag(settings)
...
class AskRequest(BaseModel):
    ...
    answer_sources: list[AnswerSource] | None = Field(default=None, max_length=3)
...
async def rag_stats(rag: RagPort = Depends(get_rag)) -> dict[str, object]:
    # stats() is an adapter diagnostic outside the portable RagPort contract.
...
async def _sse(
    ...
    *,
    ...
    - ``event: error`` / code, message and partial-char trace; then close cleanly
    ...
    try:
        if answer.memory is not None:
            ...
            if answer.memory.small_model:
                memory_payload["model_id"] = answer.memory.small_model
                memory_payload["model_display_name"] = answer.memory.small_model
            yield _sse_frame(
                "memory.sources",
...
async def _sse_after_retrieval(
    request: Request,
    answer_factory: Callable[[], Awaitable[AnswerStream]],
    *,
    conversation_id: str,
    message_id: str | None,
    model_id: str | None = None,
) -> AsyncIterator[bytes]:
    """Open SSE immediately, keep it alive during LLM Search, then stream the answer."""

    answer_task = asyncio.create_task(answer_factory(), name="rag-retrieve-and-answer")
    try:
        yield b": retrieval-started\n\n"
        while not answer_task.done():
            done, _ = await asyncio.wait(
                {answer_task},
                timeout=_RAG_RETRIEVAL_HEARTBEAT_S,
            )
            if done:
                break
            if await request.is_disconnected():
                raise asyncio.CancelledError
            yield b": retrieval-pending\n\n"
        ...
        logger.exception("RAG retrieval failed before answer streaming")
        error = RagAnswerErrorEvent(
            trace=RagAnswerErrorTrace(phase="retrieval", partial_chars=0),
        )
        yield _sse_frame(error.type, error.model_dump_json())
    finally:
...
async def rag_ask(
    ...
    memory_scope = MemoryScope.from_principal(current_principal())

    async def make_answer() -> AnswerStream:
        memory_recall = None
        if settings.memory_enabled and (
            body.answer_sources is None or "memory" in body.answer_sources
        ):
        ...
        return await retrieve_and_answer(
            ...
            rag_document_ids=(
                [f"meeting-{body.meeting_id}"] if body.meeting_id is not None else None
            ),
            answer_sources=body.answer_sources,
        )

    return StreamingResponse(
        _sse_after_retrieval(
            request,
            make_answer,
            conversation_id=body.conversation_id,
            message_id=body.message_id,
            model_id=settings.llm_main_model,
        ),
