...
def _collect_log_files() -> list[Path]:
    ...
    # The Electron main process owns its own bridge log.  Include it when the
    # canonical macOS user-data root is present so download/IPC failures and
    # backend failures can be correlated in one exported bundle.
    electron_main = Path.home() / "Library" / "Application Support" / "EchoDesk" / "logs" / "main.log"
    if electron_main.is_file() and electron_main not in out:
        out.append(electron_main)
    return out
