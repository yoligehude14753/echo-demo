...
def _minutes_audit(event: str, **fields: object) -> None:
    """Emit metadata-only success/failure records for the durable minutes flow."""

    payload = {
        "event": f"minutes.{event}",
        "at": datetime.now(UTC).isoformat(),
        **fields,
    }
    logger.info("minutes.audit %s", json.dumps(payload, ensure_ascii=False, sort_keys=True))
...
def _minutes_dto(minutes: MeetingMinutes) -> MeetingMinutes:
    if minutes.created_at.tzinfo is None:
        minutes = minutes.model_copy(
            update={"created_at": minutes.created_at.replace(tzinfo=UTC)}
        )
...
def get_meeting_pipeline_for_lifespan(
    ...
    """Build the scoped meeting overlay without an independent audio/STT path."""
    from app.api.deps import (
...
def bind_meeting_workflow_handlers(  # noqa: PLR0915 - explicit durable finalize lifecycle
    ...
    async def commit_progress_with_busy_retry(
        run_id: str,
        *,
        output: dict[str, Any],
        ...
        """Retry only the idempotent SQLite commit tail, never the LLM call."""

        for attempt in range(3):
            try:
                return await dispatcher.service.commit_run_progress_atomic(
                    run_id,
                    output=output,
                ...
                if "locked" not in str(exc).lower() and "busy" not in str(exc).lower():
                    raise
                # A commit error is uncommon but can be ambiguous. Read the
                # durable marker before replaying the transaction; if it is
                # already present, adopt it and skip a duplicate domain write.
                try:
                    recovered = await dispatcher.service.get_run(run_id)
                except sqlite3.OperationalError:
                    recovered = None
                if recovered is not None and all(
                    recovered.output.get(key) == value for key, value in output.items()
                ):
                    logger.warning(
                        "meeting finalize commit adopted after sqlite busy run_id=%s",
                        run_id,
                    )
                    return recovered
                if attempt == 2:
                ...
                logger.warning(
                    "meeting finalize sqlite busy; retrying commit tail run_id=%s attempt=%d",
                    run_id,
                    attempt + 1,
                )
               ...
               WHERE id = ? AND tenant_id = ? AND owner_id = ?
                 AND (
                       ...
                       OR (
                           ...
                           AND COALESCE(minutes_status, '') NOT IN ('ok', 'no_content')
    ...
    async def finalize_handler(context: WorkflowContext, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        if output.get("no_content") is True:
            return output
        ...
        if isinstance(domain_commit, dict):
            ...
            if not pipeline.get_segments(meeting_id):
                ...
                if not loaded:
                    ...
                    async def write_no_content(conn: aiosqlite.Connection) -> bool:
                        principal = current_principal()
                        changed = await conn.execute(
                            """UPDATE meetings
                               SET state = 'ended', ended_at = COALESCE(ended_at, ?),
                                   minutes_status = 'no_content', minutes_error = '',
                                   minutes_generation_run_id = NULL,
                                   minutes_generation_cancelled_at = NULL
                               WHERE id = ? AND tenant_id = ? AND owner_id = ?
                        ...
                        if not applied:
                            raise MeetingPipelineError(
                                "stale meeting no-content generation"
                            )
                        return True

                    committed = await commit_progress_with_busy_retry(
                        context.run_id,
                        output=output,
                        domain_writer=write_no_content,
                        domain_events=[
                            EchoEvent(
                                type="minutes.skipped",
                                meeting_id=meeting_id,
                                payload={"reason": "no_recognized_content"},
                            )
                        ],
                        message="会议没有可总结的有效转写",
                    )
                    if committed is None or committed.is_terminal:
                        raise MeetingPipelineError(
                            "meeting workflow disappeared before no-content commit"
                        )
                    return output
...
async def dispatch_meeting_finalize(  # noqa: PLR0915 - explicit winner adoption lifecycle
    ...
    *,
    ...
    if meeting is not None and meeting.minutes_status == "no_content":
        raise MeetingPipelineError(
    ...
    try:
        ...
        _minutes_audit(
            ...
            workflow_updated_at=run.updated_at,
        )
    ...
    try:
        ...
        _minutes_audit(
            ...
            meeting_id=meeting_id,
            run_id=exc.run_id,
            workflow_state=exc.state,
            error_type=type(exc).__name__,
            error=str(exc)[:1600],
        )
        raise MeetingPipelineError(str(exc)) from exc
    if done.output.get("no_content") is True:
        _minutes_audit(
            "generate.skipped",
            meeting_id=meeting_id,
            run_id=done.run_id,
            reason="no_content",
            workflow_created_at=done.created_at,
            workflow_started_at=done.started_at,
            workflow_finished_at=done.finished_at,
        )
        raise MeetingPipelineError(
            f"meeting {meeting_id} has no segments",
            code="no_content",
        )
    minutes = MeetingMinutes.model_validate(done.output["minutes"])
    _minutes_audit(
        "generate.succeeded",
        meeting_id=meeting_id,
        run_id=done.run_id,
        title=minutes.title,
        duration_sec=minutes.duration_sec,
        section_count=len(minutes.sections),
        todo_count=len(minutes.todos),
        created_at=minutes.created_at.isoformat(),
        workflow_created_at=done.created_at,
        workflow_started_at=done.started_at,
        workflow_finished_at=done.finished_at,
        workflow_updated_at=done.updated_at,
    )
    return minutes
...
async def get_minutes(
    ...
    try:
        ...
        # The persisted payload and parser detail are private implementation
        # data.  Keep the client response generic; operators get only the
        # error class, never the model/provider response body.
        logger.warning("meeting minutes payload unavailable error_type=%s", type(error).__name__)
        raise HTTPException(status_code=502, detail="minutes unavailable") from None
...
async def download_minutes_markdown(
    ...
    _minutes_audit("download.requested", meeting_id=meeting_id)
    meeting = await repository.get_meeting(meeting_id)
    if meeting is None:
        _minutes_audit("download.failed", meeting_id=meeting_id, failure_kind="meeting_not_found")
        raise HTTPException(status_code=404, detail="meeting not found")
    if not meeting.minutes_json:
        _minutes_audit("download.failed", meeting_id=meeting_id, failure_kind="minutes_missing")
    ...
    try:
        ...
        _minutes_audit(
            "download.failed",
            meeting_id=meeting_id,
            failure_kind="minutes_json_invalid",
...
async def finalize(
    ...
    # Finalize decision order is deliberately scoped before it is stateful:
    # 1. the repository lookup above hides resources from other principals;
    # 2. a completed result is an idempotent replay, not a new execution;
    # 3. an active run in this principal scope must be joined/validated by the
    #    workflow ownership contract;
    # 4. only a genuinely new execution needs a fresh intent authorization.
    if rec.minutes_json and rec.minutes_status == "ok":
        return _minutes_dto(MeetingMinutes.model_validate_json(rec.minutes_json))
    ...
    if active is None:
        ...
        if (
            ...
            raise HTTPException(
                status_code=409,
                detail="meeting summary requires an authorized intent plan",
            )
    try:
