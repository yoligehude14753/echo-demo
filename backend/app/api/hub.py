...
class SyncHistoryEntityCounts(BaseModel):
    meeting: int = Field(ge=0)
    transcript_segment: int = Field(ge=0)
    meeting_summary: int = Field(ge=0)
    artifact: int = Field(ge=0)


class SyncHistoryManifestResponse(BaseModel):
    cursor: int = Field(ge=0)
    entity_counts: SyncHistoryEntityCounts

    @classmethod
    def from_record(
        cls, record: SyncHistoryManifestRecord
    ) -> SyncHistoryManifestResponse:
        return cls(
            cursor=record.cursor,
            entity_counts=SyncHistoryEntityCounts(
                meeting=record.meeting_count,
                transcript_segment=record.transcript_segment_count,
                meeting_summary=record.meeting_summary_count,
                artifact=record.artifact_count,
            ),
        )
...
class SyncChangesResponse(BaseModel):
    ...
    manifest: SyncHistoryManifestResponse
    reset_required: Literal[True] | None = None


class SyncSnapshotResponse(BaseModel):
    cursor: int
    manifest: SyncHistoryManifestResponse
    meetings: list[SyncChangeResponse]
    transcript_segments: list[SyncChangeResponse]
    meeting_summaries: list[SyncChangeResponse]
    artifacts: list[SyncChangeResponse]
    ...
    def from_record(cls, record: SnapshotResult) -> SyncSnapshotResponse:
        ...
        return cls(
            ...
            manifest=SyncHistoryManifestResponse(
                cursor=record.cursor,
                entity_counts=SyncHistoryEntityCounts(
                    meeting=len(record.meetings),
                    transcript_segment=len(record.transcript_segments),
                    meeting_summary=len(record.meeting_summaries),
                    artifact=len(record.artifacts),
                ),
            ),
            meetings=convert(record.meetings),
            transcript_segments=convert(record.transcript_segments),
            meeting_summaries=convert(record.meeting_summaries),
            artifacts=convert(record.artifacts),
...
async def get_sync_changes(
    ...
    manifest = SyncHistoryManifestResponse.from_record(
        await store.history_manifest(principal)
    )
    return SyncChangesResponse(
        cursor=returned_cursor,
        changes=[SyncChangeResponse.from_record(record) for record in records],
        manifest=manifest,
        reset_required=True if cursor > manifest.cursor else None,
    )
