...
async def _probe_openai_models(base_url: str, api_key: str, model: str) -> ProbeResult:
    ...
    if not api_key or api_key == "EMPTY":
        return ProbeResult(ok=None, reason="no_api_key", model=model)
    ...
    try:
        ...
        return ProbeResult(ok=False, error=f"http_{resp.status_code}", model=model)
    except httpx.TimeoutException:
        return ProbeResult(ok=False, error="timeout", model=model)
...
async def _probe_all(settings: Settings) -> dict[str, ProbeResult]:
    ...
    host, port = _host_port_from_url(gateway_base_url)
        ...
        (
            ...
            asyncio.ensure_future(
                _probe_openai_models(
                    gateway_base_url,
                    settings.resolved_model_gateway_api_key,
...
async def start_prober() -> None:
    """lifespan 启动时调用。探针在后台跑，不能阻塞 backend 对外可用。"""
    ...
    logger.info(
        "prober started: interval=%ds timeout=%ds grace=%d",
        int(_PROBE_INTERVAL_S),
        int(_PROBE_TIMEOUT_S),
        _PROBE_FAILURE_GRACE_COUNT,
    )
...
def _db_status(settings: Settings) -> dict[str, Any]:
    ...
    # 跟 repo/__init__.py、ambient_capture、text catalog 等保持一致 defensive expand
