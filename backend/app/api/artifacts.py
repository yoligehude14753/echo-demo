...
_DIAGNOSTIC_ID_RE = re.compile(r"\bdiagnostic_id=([A-Za-z0-9._-]+)")


def _artifact_audit(event: str, **fields: object) -> None:
    """Write one metadata-only generation/download audit record.

    The workflow/event tables remain the durable source of truth.  This log is
    intentionally value-light (no brief, transcript, or generated document
    body) but carries enough IDs/timestamps to correlate a user action with
    its terminal state and file transfer.
    """

    payload = {
        "event": f"artifact.{event}",
        "at": datetime.now(UTC).isoformat(),
        **fields,
    }
    _log.info("artifact.audit %s", json.dumps(payload, ensure_ascii=False, sort_keys=True))
    ...
    *,
    ...
    async def handler(context: WorkflowContext, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        await event_bus.publish(
            EchoEvent(
                ...
                payload={
                    ...
                    "run_id": context.run_id,
                    "todo_id": todo_id,
                    # This is the model requested by the current gateway
                    # contract. The terminal artifact.ready event still wins
                    # when the provider returns its observed model identity.
                    "model": settings.llm_main_model,
                    "model_display_name": settings.llm_main_model,
                },
            )
        )
        ...
        try:
            ...
            diagnostic_match = _DIAGNOSTIC_ID_RE.search(error)
            _artifact_audit(
                "generate.failed",
                run_id=context.run_id,
                artifact_type=artifact_type,
                meeting_id=meeting_id,
                todo_id=todo_id,
                failure_kind="remote_llm" if isinstance(exc, LLMError) else "skill",
                error_type=type(exc).__name__,
                error=error,
                diagnostic_id=diagnostic_match.group(1) if diagnostic_match else None,
            )
            failed_events = [
                EchoEvent(
        ...
        if committed is None:
            _artifact_audit(
                ...
                meeting_id=meeting_id,
                todo_id=todo_id,
                failure_kind="workflow_disappeared",
        ...
        _artifact_audit(
            ...
            workflow_created_at=committed.created_at,
            workflow_started_at=committed.started_at,
            workflow_finished_at=committed.finished_at,
            workflow_updated_at=committed.updated_at,
        )
        return output
...
async def generate(
    ...
    idempotency_key = f"artifact.generate:{digest}"
    existing = await dispatcher.service.get_by_idempotency(idempotency_key)
    if existing is not None:
        try:
            done = (
                existing
                if existing.state == "succeeded"
                else await dispatcher.wait_succeeded(existing.run_id)
            )
            ...
            _artifact_audit(
                ...
                failure_kind="workflow_execution",
        ...
        artifact = done.output.get("artifact")
        if not isinstance(artifact, dict):
            raise InternalHTTPException(
                status_code=500,
                detail="artifact workflow result is unavailable",
            )
        _artifact_audit(
            "generate.replayed",
            run_id=done.run_id,
...
async def download(
    ...
    _artifact_audit("download.requested", artifact_id=artifact_id)
    if not _ARTIFACT_ID_RE.fullmatch(artifact_id):
        _artifact_audit(
            "download.failed", artifact_id=artifact_id, failure_kind="invalid_artifact_id"
        )
    ...
    try:
        ...
        _log.warning(
            "artifact db lookup failed artifact_id=%s error_type=%s",
            artifact_id,
            type(e).__name__,
        )
        artifact = None
    if artifact is not None:
        ...
        if f is not None:
            download_name = _download_name_from_artifact(artifact, f)
            _artifact_audit(
                "download.ready",
    ...
    if settings.public_demo_mode:
        _artifact_audit(
            "download.failed",
            artifact_id=artifact_id,
            failure_kind="artifact_not_found_public",
        )
        raise HTTPException(status_code=404, detail="artifact not found")
    ...
    if build_dir == base or base not in build_dir.parents or not build_dir.is_dir():
        _artifact_audit(
            "download.failed", artifact_id=artifact_id, failure_kind="build_dir_missing"
        )
        raise HTTPException(status_code=404, detail="artifact not found")
    candidates = list(build_dir.glob("output.*"))
    if not candidates:
        _artifact_audit(
            "download.failed", artifact_id=artifact_id, failure_kind="output_missing"
        )
