...
#
# 模型 endpoint 与凭证由 EchoDesk owner 的 Model Gateway 合同唯一管理；
# SettingsPanel 只保留不改变模型路由的业务参数。
#
# 安全策略：
...
_REMOTE_PATCH_ALIASES: dict[str, str] = {}
_REMOTE_SOURCE_ALIASES: dict[str, tuple[str, ...]] = {}
_ALLOWED_KEYS = {f[0] for f in _REMOTE_FIELDS} | set(_REMOTE_PATCH_ALIASES)
...
def get_remote_settings(
    ...
    for key, attr, sensitive in _REMOTE_FIELDS:
        ...
        source_keys = (key, *_REMOTE_SOURCE_ALIASES.get(key, ()))
        items.append(
            RemoteFieldDTO(
                key=key,
                value=display,
                ...
                source=(
                    "user"
                    if any(source_key in user_overrides for source_key in source_keys)
                    else "default"
                ),
            )
        )
...
def patch_remote_settings(body: RemoteSettingsPatch) -> dict[str, object]:
    ...
    # 旧版客户仍可提交 provider 私有字段名，持久化时统一写入标准名。
    # 同一请求同时传新旧名且值冲突时拒绝，避免顺序覆盖。
    sanitized: dict[str, str] = {}
    for key, value in body.updates.items():
        canonical = _REMOTE_PATCH_ALIASES.get(key, key)
        if canonical in sanitized and sanitized[canonical] != value:
            raise HTTPException(
                status_code=422,
                detail=f"配置项 {canonical} 的新旧字段值冲突",
            )
