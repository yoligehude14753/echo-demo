...
class BuiltinIntentPlan(BaseModel):
    ...
    answer_sources: list[AnswerSource] = Field(default_factory=list, max_length=4)
    ...
    def _validate_target(self) -> BuiltinIntentPlan:
        ...
        if len(set(self.answer_sources)) != len(self.answer_sources):
            raise ValueError("answer_sources cannot contain duplicates")
        if self.execution_target == "conversation":
            if self.answer_sources not in ([], ["direct_model"]):
                raise ValueError("conversation only supports direct_model")
        elif "direct_model" in self.answer_sources:
            raise ValueError("direct_model cannot be combined with retrieval sources")
        if self.builtin_intent in {"search_rag", "search_web"}:
            if self.answer_sources and not any(
                source in {"memory", "local_documents", "web"}
                for source in self.answer_sources
            ):
                raise ValueError("search intent requires at least one retrieval source")
        return self
