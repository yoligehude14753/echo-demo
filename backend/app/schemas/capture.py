...
#   terminal_ignored → meeting 已终止；不调用 STT、不追加 segment，receipt 可完成
...
class CaptureAdmissionReceipt(BaseModel):
    """客户端可消费的、非敏感 durable capture 接纳回执。"""
...
class CaptureChunkResult(BaseModel):
    ...
    # 仅供服务端 durable receipt 关联规范行；永不进入 HTTP payload。
    ambient_segment_id: int | None = Field(default=None, exclude=True)
    ...
    speaker_status: Literal["pending", "identified", "unknown", "failed"] | None = None
    ...
    source: Literal["desktop", "device"] | None = None
    capture_mode: CaptureStreamMode = "free"
    # 只有 capture_request_receipts 的 completion CAS 成功后才填充；处理中/失败响应不得伪造。
    admission: CaptureAdmissionReceipt | None = None
