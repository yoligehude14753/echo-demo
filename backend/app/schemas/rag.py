...
class RagDocumentEntry(BaseModel):
    """Model-search catalog row; synopsis input is never returned as evidence."""
    ...
    n_chunks: int = 0
    # Historical field name retained for wire/test compatibility. It now holds
    # a bounded synopsis (meeting minutes, or head/tail source synopsis), not a
    # ranking preview.
    preview: str = ""
...
class RagAnswerErrorTrace(BaseModel):
    phase: Literal["retrieval", "generation"] = "generation"
    partial_chars: int = 0
