...
from app.agents.agentos import AGENTOS_SUBMIT_MAX_WALL_S, AgentOSBackend, submit_operation_key
...
class AgentTaskBackend(Protocol):
    """Runtime boundary for the legacy HTTP-compatible runner."""
...
class AgentTaskService:
    ...
    def __init__(
        ...
        *,
        ...
        backend: AgentTaskBackend | None = None,
        ...
        self.backend: AgentTaskBackend = backend or AgentOSBackend(settings)
    ...
    async def _insert_task_tx(
        ...
        *,
        ...
        # Keep the requested runner model visible in the durable task snapshot;
        # terminal runner events may later replace it with the observed model.
        snapshot["model"] = intent.runner_model or self.settings.llm_main_model
        snapshot["route"] = result.provider or RUNNER_CLAUDE_CODE
        ...
        envelope = {
            ...
            "runner": {
                ...
                "base_url": intent.runner_base_url or self.settings.model_gateway_base_url,
            },
        }
        cur = await conn.execute(
    ...
    async def _run_bridge(self, rec: AgentTaskRecord) -> None:  # noqa: PLR0912, PLR0915
        ...
        try:
            ...
            # Legacy tasks poll the explicitly configured AgentOS HTTP
            # compatibility endpoint. The removed Electron runtime protocol
            # must not become an implicit fallback or a packaged dependency.
            await asyncio.sleep(self._bridge_recovery_interval_seconds)
        except asyncio.CancelledError:
            retry = False
            raise
...
def get_agent_task_service(settings: Settings, event_bus: InMemoryEventBus) -> AgentTaskService:
    ...
    if _service is None:
        from app.agents.packaged_llm import PackagedLLMAgentBackend
        ...
        _service = AgentTaskService(
            ...
            backend=PackagedLLMAgentBackend(settings, get_llm_singleton(settings)),
        )
