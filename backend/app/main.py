...
启动端点由 lifecycle owner 注入：
    ECHODESK_BASE_URL=http://<host>:<assigned-port> backend/start.sh

应用不内置本地端口。启动后会发布同一 runtime endpoint，供独立 harness
读取；未注入端点时 fail closed。
"""
...
from app.runtime_endpoint import clear_runtime_endpoint, publish_runtime_endpoint
...
_CORS_EXPOSE_HEADERS = f"{PUBLIC_MINIMUM_CLIENT_VERSION_HEADER}, Link"
...
def _cors_headers_for_server_error(request: Request, settings: Settings) -> dict[str, str]:
    """为 ServerErrorMiddleware 生成的响应补齐全局 CORS 响应头。

    Starlette 的 server-error 边界位于应用中间件外侧，因此异常处理器返回的响应
    不会再流经 ``CORSMiddleware``。这里只回显配置中精确允许的 Origin。
    """

    origin = request.headers.get("origin")
    if origin is None or origin not in settings.allowed_origins_list:
        return {}
    return {
...
async def lifespan(_app: FastAPI) -> AsyncIterator[None]:  # noqa: PLR0912, PLR0915
    ...
    endpoint_record = publish_runtime_endpoint(settings.echodesk_base_url)
    _app.state.ready = True
    yield

    # 顺序：先停恢复器/执行器，再销毁它们依赖的 LLM、event bus 与 repository。
    _app.state.ready = False
    clear_runtime_endpoint(endpoint_record, base_url=settings.echodesk_base_url)
...
def _install_error_handlers(app: FastAPI, settings: Settings) -> None:
    ...
    async def asr_error(_request: Request, exc: ASRError) -> JSONResponse:
        ...
        if status_code >= 500:
            # Machine-readable, payload-free diagnostics for capture retries.
            # The safe provider code is already bounded by the ASR contract.
            headers["X-Capture-Error-Class"] = exc.machine_code
            logger.warning(
                "capture_error_class=%s source_branch=asr_error",
                exc.machine_code,
            )
    ...
    async def unhandled_error(request: Request, exc: Exception) -> JSONResponse:
        ...
        cors_headers = _cors_headers_for_server_error(request, settings)
        logger.error(
        ...
        if protect_private_response:
            return JSONResponse(
                ...
                status_code=500,
                headers={**PRIVATE_NO_STORE_HEADERS, **cors_headers},
            )
        return JSONResponse(
        ...
        response.update(
            {
                ...
                # Local mode never issues a bearer session, but the renderer
                # still needs the server-owned capture identity.  Project it
                # from the authoritative principal instead of letting a
                # client fabricate a device or owner scope.
                "principal": {
...
def create_app() -> FastAPI:  # noqa: PLR0915 - application composition root
    ...
    async def bind_request_identity(
        ...
        try:
            ...
            diagnostics = runtime_registry.diagnostics()
            logger.warning(
                "capture_error_class=runtime_capacity_exceeded source_branch=request_runtime_capacity "
                "configured_limit=%d in_use=%d active_leases=%d rejected=%d",
                diagnostics["configured_limit"],
                diagnostics["in_use"],
                diagnostics["active_leases"],
            ...
            response = JSONResponse(
                ...
                headers={
                    "Retry-After": "1",
                    "X-Capture-Error-Class": "runtime_capacity_exceeded",
                },
            )
        finally:
            if quota_entered:
