...
class RagPort(Protocol):
    ...
    async def search_catalog(self) -> list[RagDocumentEntry]:
        """Return every visible document with a bounded plain-text preview."""

    async def read_chunks(self, doc_ids: list[str]) -> list[RagChunk]:
        """Return original evidence chunks for the selected visible documents."""
