...
为什么需要：
- FireRedASR2 (heyi :8090) 的 OpenAPI schema 只接受 file / model / language /
...
- 既然 STT 服务侧无法直出标点，就在 ambient 主链路里加一个轻量 LLM 后处理：
  Qwen3-1.7B on heyi-bj :7860（与 llm_fast 同通道），把多段 raw text **一次性**
...
- **批 batch**：单 chunk 通常只有 1-3 段，一次 LLM 调用 < 300 tokens prompt +
  < 500 tokens completion，p50 < 700ms (Qwen3-1.7B local)，配 2s 超时绰绰有余。
