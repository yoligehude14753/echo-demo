...
def _check_html_one_pager_invariants(html: str) -> list[str]:
    ...
    3. 不许 ``rgba(``（WeasyPrint 双 rect bug；invariant #8）
    4. 必须含 ``#f5f4ed`` 或 ``var(--parchment)``（背景锚点；invariant #1）
    5. 不许日文片假名（M2.7 偶发；不在 Kami 10 invariants 但在 INTEGRATE_PROMPT 6.6）
    6. 不许 emoji（编辑设计语言禁用 emoji；SKILL.md 内容质量门禁）
...
class SkillExecutor:
    ...
    def __init__(self, settings: Settings) -> None:
        ...
        self._diagnostic_root = Path(
            getattr(settings, "skill_executor_diagnostic_dir", self._build_root / "diagnostics")
        ).expanduser()
    ...
    async def _generate_via_default_pipeline(
        ...
        *,
        ...
        result = await self._exec_for_kind(
            kind,
            code,
            build_dir,
            ext,
            diagnostic_context={
                ...
                "http_status": llm_out.http_status,
                "output_limit": self._max_tokens,
                "output_limit_reached": llm_out.completion_tokens >= self._max_tokens,
            },
        )
        retry_count = 0
        while (
    ...
    async def _generate_html_one_pager(
        ...
        *,
        ...
        - invariants 不满足（rgba / emoji / 没 #f5f4ed 等）→ ``SkillError("invariant 违反: ...")``
        ...
        *,
        ...
        if kind == "pdf":
            ...
            return await exec_python_to_artifact(
                ...
                timeout_s=self._timeout_s,
                env=env,
                diagnostic_dir=self._diagnostic_root,
