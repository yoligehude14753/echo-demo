...
- HTML（新）: Kami warm-parchment serif；LLM 直出整篇 HTML（inline SVG 按内容需要可选）
...
[ "数据" 一节 —— 标题左侧 4px 墨蓝竖线 ]
  ...
  - 数据适合可视化时可加入 inline SVG bar / line chart；不适合时使用表格或 KPI 卡

[ "产品 / 业务矩阵" 一节 —— Kami 风格的"边框 + 浅灰背景"卡片，不要 box-shadow ]

[ "竞争与风险" 一节 —— 时间序列 / bullet 风格，SVG 可选 ]
...
- inline SVG 可选；仅在能表达真实数据时使用，不得为满足数量凑图。使用时保持 Kami 风：极细 stroke 1px，无 fill 阴影
