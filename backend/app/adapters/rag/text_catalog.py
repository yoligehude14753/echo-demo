"""Text-catalog persistence adapter for documents and meeting transcripts.

The adapter chunks and durably stores original text, but performs no ranking or
retrieval.  Local evidence selection belongs exclusively to Qwen LLM Search.
Historical SQLite/cache identifiers are retained only to open existing user
data without a destructive migration.
"""
...
from app.adapters.rag.text_store import IndexSnapshot, TextCatalogStore
from app.config import Settings
from app.schemas.rag import RagChunk, RagDocumentEntry
...
class _IndexedChunk:
    """Deeply immutable in-memory representation of one committed text chunk."""
...
class TextCatalogRag:
    """Durable original-text catalog implementing ``ports.rag.RagPort``."""
    ...
    def _list_docs_sync(self) -> list[dict[str, object]]:
        return [
            entry.model_dump(exclude={"preview"})
            for entry in self._search_catalog_sync()
        ]

    async def search_catalog(self) -> list[RagDocumentEntry]:
        async with self._lock:
            return await asyncio.to_thread(self._search_catalog_sync)

    def _search_catalog_sync(self) -> list[RagDocumentEntry]:
        scope = _scope()
        snapshot = self._snapshot_for_scope(scope)
        ...
        for chunk in self._visible_chunks_for_scope(snapshot, scope):
            ...
            if kind == "meeting" and "【逐字稿】" in first_text:
                first_text = first_text.split("【逐字稿】", 1)[0]
                first_text = first_text.removeprefix("【纪要】").strip()
            doc = agg.setdefault(
                chunk.doc_id,
                {
                    "doc_id": chunk.doc_id,
                    ...
                    "n_chunks": 0,
                    "preview_first": first_text[:1_200],
                    "preview_last": "",
                },
            )
            doc["n_chunks"] = int(doc["n_chunks"]) + 1
            if kind != "meeting":
                doc["preview_last"] = chunk.text[-600:]

        updated_at: dict[str, str] = {}
        with sqlite3.connect(str(self._store.db_path), timeout=30.0) as conn:
            rows = conn.execute(
                """SELECT doc_id, source, source_path, updated_at
                   FROM rag_documents
                   WHERE tenant_id = ? AND owner_id = ? AND status = 'ready'""",
                scope,
            ).fetchall()
        for doc_id, source, source_path, timestamp in rows:
            key = str(doc_id)
            updated_at[key] = str(timestamp)
            if key in agg:
                agg[key]["source"] = str(source or agg[key]["source"])
                agg[key]["source_path"] = source_path or agg[key]["source_path"]

        entries: list[RagDocumentEntry] = []
        for doc_id, doc in agg.items():
            first = str(doc.pop("preview_first", "")).strip()
            last = str(doc.pop("preview_last", "")).strip()
            synopsis = first
            if last and last != first and last not in first:
                synopsis = f"{first}\n【文末摘录】\n{last}" if first else last
            entries.append(
                RagDocumentEntry(
                    **doc,
                    preview=synopsis[:2_000],
                    updated_at=updated_at.get(doc_id),
                )
            )
        return sorted(entries, key=lambda item: item.updated_at or "", reverse=True)

    async def read_chunks(self, doc_ids: list[str]) -> list[RagChunk]:
        async with self._lock:
            return await asyncio.to_thread(self._read_chunks_sync, doc_ids)

    def _read_chunks_sync(self, doc_ids: list[str]) -> list[RagChunk]:
