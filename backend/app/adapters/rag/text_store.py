"""Cross-process commit store for original-text JSON and its revision manifest."""
...
class TextCatalogStoreError(RuntimeError):
    """The durable text catalog manifest or its cache failed an integrity check."""
...
def _payload_fields(
    ...
    if not doc_id:
        raise TextCatalogStoreError("text catalog payload is missing doc_id")
...
def _atomic_write(path: Path, content: bytes) -> None:
    ...
    temp = path.parent / f".text-catalog-{path.name}-{uuid4().hex}.tmp"
    try:
...
class TextCatalogStore:
    """SQLite-coordinated payload manifest for one physical text catalog directory."""
    ...
    def __init__(
        ...
        *,
        ...
        if max_scope_payload_bytes < 1:
            raise ValueError("text catalog scope payload limit must be positive")
    ...
    def _safe_index_path(self, raw: Path | str) -> Path:
        ...
        if path.parent != self.index_dir or path.suffix.lower() != ".json":
            raise TextCatalogStoreError(f"text catalog cache path escapes index directory: {path}")
        return path
    ...
    def _normalize_payload(self, payload: Payload, revision: int) -> Payload:
        ...
        if not isinstance(cloned, dict):  # pragma: no cover - input type is already a dict
            raise TextCatalogStoreError("text catalog payload is not an object")
        ...
        # Keep the historical payload key so existing caches remain readable.
    ...
    def _revision(self, conn: sqlite3.Connection) -> int:
        ...
        if row is None:
            raise TextCatalogStoreError("text catalog state row is missing")
        return int(row[0])
    ...
    def _upsert_manifest(
        ...
        if not self._table_exists(conn, "rag_documents"):
            raise TextCatalogStoreError("rag_documents manifest table is missing")
        ...
        try:
            ...
            raise TextCatalogStoreError(f"rag_documents manifest rejected {doc_id}: {exc}") from exc
    ...
    def _insert_document(
        ...
        if next_payload_bytes > self.max_scope_payload_bytes:
            raise TextCatalogStoreError(
                "text catalog principal data exceeds payload limit: "
    ...
    def _repair_orphan_caches(
        ...
        *,
        ...
        for pattern in (".text-catalog-*.tmp", ".bm25-*.tmp"):
            for temp in self.index_dir.glob(pattern):
                temp.unlink(missing_ok=True)
        ...
        with self._connect() as conn:
            ...
            for row in rows:
                ...
                if content_hash != str(row["content_hash"]):
                    raise TextCatalogStoreError(
                        f"text catalog durable payload hash mismatch: {row['doc_id']}"
                    )
                payload = json.loads(raw)
                if not isinstance(payload, dict):
                    raise TextCatalogStoreError(
                        f"text catalog durable payload is not an object: {row['doc_id']}"
                    )
                index_path = self._safe_index_path(str(row["index_path"]))
        ...
        *,
        ...
        try:
            with self._connect() as conn:
                ...
                if row is not None:
                    ...
                    if not isinstance(decoded, dict):
                        raise TextCatalogStoreError(
                            f"text catalog durable payload is not an object: {doc_id}"
                        )
                    existing = decoded
                ...
                if fields[0] != tenant_id or fields[1] != owner_id or fields[3] != doc_id:
                    raise TextCatalogStoreError("text catalog mutator changed the document identity")
                revision += 1
            ...
            try:
                ...
                self.log.error("text catalog cache rollback repair failed: %s", repair_exc)
            raise


__all__ = [
    "TextCatalogStore",
    "TextCatalogStoreError",
    "IndexMutation",
    "IndexSnapshot",
    "StoredIndexDocument",
]
