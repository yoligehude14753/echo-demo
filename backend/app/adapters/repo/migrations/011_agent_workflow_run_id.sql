-- 011_agent_workflow_run_id.sql
-- 0.3：保留旧 agent_tasks 兼容表，同时把每个 Agent task 映射到 workflow_runs。
-- 真实用户库可能已经登记过旧 005 但缺 agent_tasks 表；这里先补齐兼容表。
...
ALTER TABLE agent_tasks ADD COLUMN workflow_run_id TEXT REFERENCES workflow_runs(run_id) ON DELETE SET NULL;

CREATE INDEX IF NOT EXISTS idx_agent_tasks_workflow_run
    ON agent_tasks(workflow_run_id);
