...
- 桌面端、backend 与 Android 统一推进到 `0.3.5`；Android 正式包使用单调递增的 `versionCode=30500`。
- 保留稳定版更新协议，已安装的 `0.3.4` 可识别三端各自的 `0.3.5` 标准更新资产。
- 主模型与小模型的产品展示名继续分别使用 `DeepSeek V4 Flash` 与 `Qwen3 8B`，不暴露 provider 内部名称。
...
- macOS 打包版在公共后端模式下改由 Electron 本机进程管理工作区目录，升级后继续读取本机 `workspaces.json`。
- 工作区重扫由本机进程递归读取授权目录，再以 `source=workspace` 上传到后端 RAG，保留原始 `source_path` 便于展示、去重和清理。
- `/rag/ingest` 支持工作区来源，并按 `workspace_max_file_mb=100` 处理工作区扫描文件；用户拖入上传仍保留独立上传限制。
- 设置页将“单文件上限”改为扫描保护说明，避免误解为知识库目录丢失或只能添加单文件。
- 版本号同步到 `0.2.50`；Android / TV `versionCode=250`、`versionName=0.2.50`。
...
- 采集上传失败提示改为限时展示，不再用“后端连接断开”误导为整站故障。
- `/capture/stats` 诊断轮询成功后会主动清理采集上传失败提示，避免静音/底噪期间没有新 chunk 成功回包而无法自动消失。
- 版本号同步到 `0.2.48`；Android / TV `versionCode=248`、`versionName=0.2.48`。

## [0.2.47] – 2026-07-09

桌面端更新入口补齐：发现新版本时，在主界面顶栏直接显示“更新”，用户点击即可进入安装链路。

### 新增

- 顶栏预留更新入口：无更新时隐藏；有可更新版本时显示“更新”，下载完成后显示“安装并重启”。
- 顶栏更新入口复用后台更新状态和安装 IPC；可自动安装的平台点击后下载安装，不能自动安装的平台打开 Release 下载页。

### 变更

- 下载完成后的更新确认从弹窗主导改为顶栏入口主导，避免遮挡当前工作流。
- 版本号同步到 `0.2.47`；Android / TV `versionCode=247`、`versionName=0.2.47`。

## [0.2.46] – 2026-07-09

桌面端更新体验改进：后台定时检测新版本，发现可自动安装的桌面包后先下载，下载完成再由用户确认重启安装。

### 新增

- macOS 桌面端启动后会延迟检查更新，并按固定间隔在后台继续检查。
- 发现可一键安装的新版本时会后台下载，下载完成后弹出“安装并重启”确认，不会在用户未确认时退出。
- 设置页更新按钮会区分下载中、已下载和手动下载 fallback；不能一键安装的平台仍打开 Release 下载页。

### 修复

- 后台更新下载失败时停在错误态，不再误触发安装流程。
- 版本号同步到 `0.2.46`；Android / TV `versionCode=246`、`versionName=0.2.46`。
...
安装入口暴露底层服务商名称，默认服务地址在设置页脱敏展示。

### 修复

- 顶栏状态从供应商名改为“模型服务 / 智能引擎”，popover 同步改成
  “语音识别 / 语音合成 / 主模型 / 联网检索”等中性文案。
- STT 熔断、纪要超时、首次启动引导等 toast / hint 不再出现“云端”字样。
- 设置页默认 Base URL 脱敏为空，仅提示“使用内置服务配置”；用户自定义私有地址仍可编辑。
- README、INSTALL、TV_INSTALL 入口统一指向 `v0.2.33` 资产。
- 版本号同步到 `0.2.33`；Android / TV `versionCode=233`、`versionName=0.2.33`。
...
再写入新版应用文件；用户数据库、工作区配置和历史数据目录不删除。

### 修复

- Windows NSIS 安装器新增 silent repair hook，避免 0.2.24 等旧安装残留导致
  `/S` 覆盖安装卡住或继续打开旧版。
- 保持 current-user 安装模式；清理范围仅限 `%LOCALAPPDATA%\Programs\EchoDesk`
  程序目录和安装注册表，不触碰 EchoDesk 用户数据目录。
