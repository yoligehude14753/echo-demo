...
Windows 和 Android 请前往 [EchoDesk v0.3.4 Release](https://github.com/yoligehude14753/echo-demo/releases/tag/v0.3.4)
下载对应安装包。macOS 当前提供 Apple Silicon（arm64）版本；该版本未经过 Apple notarization，
可选择以下任一安装方式。

### 方式 A：下载 DMG

下载 [`EchoDesk-0.3.4-arm64.dmg`](https://github.com/yoligehude14753/echo-demo/releases/download/v0.3.4/EchoDesk-0.3.4-arm64.dmg)，
打开后把 `EchoDesk.app` 拖到“应用程序”。首次启动请在 Finder 中右键 EchoDesk，选择“打开”。
若仍显示“应用已损坏”，请使用方式 B 或 C。

### 方式 B：Homebrew Cask

需要先安装 [Homebrew](https://brew.sh/)。EchoDesk 当前未公证，因此安装时使用
`--no-quarantine`：
...
若提示旧 App 已存在，先备份旧 App 再安装；需要强制覆盖 cask 时使用：
...
把下面整段复制到 Terminal。它会下载并校验 DMG，备份旧 App，只清除 EchoDesk 自身的
quarantine，本机 ad-hoc 重签、严格验证并启动；不会运行远程脚本或关闭全局 Gatekeeper。
...
完整的升级、卸载和排障说明见 [`docs/MACOS_INSTALL.md`](docs/MACOS_INSTALL.md)。无需运行可能被
Gatekeeper 拦截的 `.command` 文件。
