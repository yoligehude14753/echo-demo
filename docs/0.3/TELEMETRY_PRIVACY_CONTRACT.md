...
状态：candidate；本任务的本地 contract 设计，生产 ingestion、persistence、cross-user query 均未实现。
...
| 同一身份跨 epoch 或 key version 可被关联 | 轮换失效，形成长期轨迹 | HMAC domain 同时包含用途、key version、subject kind 与 epoch；epoch 或 key version 变化时输出不同 pseudonym。 |
| 小 cohort 被查询反推 | 单用户行为被暴露 | query 对每个 tenant cohort 按 distinct user pseudonym 执行可配置 `k` 抑制，低于阈值不返回 aggregate，且不返回 user/device 个体 key。 |
...
| 重试/重复投递放大 request count | 成功率和用量失真 | caller token 只作为 HMAC 输入；server 用独立 event-id domain 生成 opaque `event_id`，同 token 同 payload 只计一次，冲突 payload 拒绝。 |
...
- cohort 输出：只保留 `tenant_pseudonym`；user/device pseudonym 仅在 adapter 内作为轮换后的 distinct 计算材料，不进入 aggregate/query key。
...
- 幂等与生命周期：caller token 只用于 server HMAC 派生受限 `event_id`；UTC `occurred_at` 用于 retention、duplicate suppression 和 delete audit receipt，不作为用户可见数据。
...
```text
event_id: caller-supplied opaque retry token，仅作为 HMAC 输入
...
```text
server-derived HMAC event_id, occurred_at, pseudonymous identity, key_version, epoch,
...
- 聚合 key：`epoch + key_version + tenant_pseudonym + operation + platform + app_version + provider`；不含 user/device 个体细分。
- 聚合值：`distinct_user_count`、`request_count`、`success_count`、`failure_count`、`success_rate`、typed `failure_reason_counts`、`latency_sum_ms`、`queue_wait_sum_ms`、`audio_duration_sum_ms`、`audio_duration_event_count`；failure reason 只能是 stable enum + count，不接受任意 map/text。
- `success_rate = success_count / request_count`；无事件不产生 cohort。
- query 只接受 typed `TelemetryQuery`（tenant pseudonym、产品维度和可选的 `FailureReason` enum filter），不接受 raw identity、user/device 个体 filter、自由文本、URL 或 request body；每个结果按 distinct user count 执行 `k_threshold` 抑制。
...
| T-03 | key rotation 或 epoch 变化后 pseudonym 改变，旧值不能由输出字段直接关联。 | rotation/cross-period 单测 | pseudonym 不相等；domain 含 key version；输出仅含 pseudonym/version/epoch。 |
...
| T-07 | caller event token 不直接 materialize；同 token 同 payload 重复提交。 | server event-id/idempotency 单测 | event id 为 HMAC opaque 值；request count 仍为 1；冲突 payload 明确拒绝。 |
| T-08 | 多事件按 tenant cohort 和产品维度聚合成功/失败/reason/延迟/queue/audio。 | aggregate 单测 | distinct user count、各计数、reason counts、sum、rate 精确匹配；不含 user/device 个体 key。 |
| T-09 | 同一 pseudonym 多事件或 cohort distinct user 数小于 k。 | suppression 单测 | 结果为空；k 个 distinct user 满足阈值且只返回 cohort aggregate。 |
...
```text
telemetry_contract=candidate   # 等待 Sol 指定的独立复验
