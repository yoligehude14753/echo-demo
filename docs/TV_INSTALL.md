...
- `EchoDesk-0.3.0-alpha.1-smart-tv.apk`：电视直接安装用 APK。
- `EchoDesk-0.3.0-alpha.1-smart-tv-oneclick.zip`：电脑一键安装包，含 APK、macOS 脚本、Windows PowerShell 脚本。
...
3. 查到电视 IP。
4. 解压 `EchoDesk-0.3.0-alpha.1-smart-tv-oneclick.zip`。
5. macOS：
...
1. 把 `EchoDesk-0.3.0-alpha.1-smart-tv.apk` 拷到 U 盘。
2. 在电视文件管理器里打开 APK。
3. 按提示允许安装未知来源应用。

## 服务连接

TV APK 是 EchoDesk 前端客户端，不包含 Python 服务，也不包含真实 API key。
默认连接 EchoDesk 公共演示服务：

```text
https://echodesk.yoliyoli.uk
```

模型服务（STT / TTS / Fast LLM）由 EchoDesk 服务端统一提供，客户端只连 EchoDesk 服务端；
真实 key 只在服务端配置，不会打进 APK。

电视端不内置 Python 服务：Android TV / AOSP TV 的系统权限、Python wheel、
本地文件目录和后台保活都不适合直接运行完整 FastAPI 服务。TV APK 只负责 UI、
录音、扫码保存和把音频发送到 EchoDesk 服务端；STT / TTS / LLM 都由服务端
通过受控配置处理。这样用户能直接安装使用，同时不会从 APK 里逆向出模型 key。

如需内网调试，也可以临时改成局域网服务地址。先在电脑上启动：
...
- STT / TTS / Fast LLM 访问通过 EchoDesk 服务端的受控配置完成。
