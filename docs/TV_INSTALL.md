...
- `EchoDesk-0.2.31-smart-tv.apk`：电视直接安装用 APK。
- `EchoDesk-0.2.31-smart-tv-oneclick.zip`：电脑一键安装包，含 APK、macOS 脚本、Windows PowerShell 脚本。
...
3. 查到电视 IP。
4. 解压 `EchoDesk-0.2.31-smart-tv-oneclick.zip`。
5. macOS：
...
1. 把 `EchoDesk-0.2.31-smart-tv.apk` 拷到 U 盘。
2. 在电视文件管理器里打开 APK。
3. 按提示允许安装未知来源应用。
