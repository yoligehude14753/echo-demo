...
v0.3.3 正式发布范围只包含 macOS、Windows、Android/TV。Linux 不再生成或上传正式 Release
候选，也不再作为唯一 Required `check` 的发布前置条件。
...
在 canonical repository 中建立三个 protected environment：

- `desktop-release-macos`
- `desktop-release-windows`
- `android-release`

三个 environment 都必须只允许 `main`、启用 required reviewers、禁止 admin bypass。环境规则属于 GitHub
...
### 1.5 唯一 release SHA 与并行构建/发布

三个正式 workflow 都只接受从 canonical repository `main` 触发的同一个 40 位小写
`release_sha`，并在签名 environment 前验证该 SHA 的 `ci.yml` push/main run 与 required
`check` 成功：
...
gh workflow run build-desktop-release-candidates.yml \
  --repo yoligehude14753/echo-demo --ref main \
  -f release_sha="${release_sha}"
...
桌面 workflow 内的 macOS 与 Windows job 会并行运行；Android/TV workflow 可与桌面 workflow
并行运行。候选 artifact 名称分别带有该 `release_sha`，失败或 SHA 不一致时不会产生可发布
候选。

候选全部成功后，对同一 GitHub Release tag 和同一 `release_sha`，按平台并行触发：

```bash
gh workflow run publish-release-assets.yml \
  --repo yoligehude14753/echo-demo --ref main \
  -f release_tag="${release_tag}" -f release_sha="${release_sha}" \
  -f source_run_id="${macos_run_id}" -f platform=macos
gh workflow run publish-release-assets.yml \
  --repo yoligehude14753/echo-demo --ref main \
  -f release_tag="${release_tag}" -f release_sha="${release_sha}" \
  -f source_run_id="${windows_run_id}" -f platform=windows
gh workflow run publish-release-assets.yml \
  --repo yoligehude14753/echo-demo --ref main \
  -f release_tag="${release_tag}" -f release_sha="${release_sha}" \
  -f source_run_id="${android_run_id}" -f platform=android
```

`publish-release-assets.yml` 会再次校验 source run、目标 job、tag commit、artifact allowlist
和 checksum；不接受 Linux、unsigned/ad-hoc artifact、不同 SHA 或未完成的 run。
