...
| 后端 ID | 部署位置 | URL | 状态 |
|---|---|---|---|
| **`cosyvoice`** (主) | heyi-bj :8094 (fasterqwen3-tts 共址) | `http://100.87.251.9:8094` | TTFA < 500ms，stream 走通 |
| `openai` (备) | Yunwu | `https://yunwu.ai/v1/audio/speech` | 按 token 计费 |

注：echo 仓库的 `.env.heyi.example` 写 :8092，但 `config.py` 默认 `:8094`，**以 `:8094` 为准**（最新部署）。
...
- backend `app/adapters/tts/heyi_cosyvoice.py` 默认走 :8094
