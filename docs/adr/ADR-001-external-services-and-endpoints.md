...
> 更新：2026-06-18，STT/TTS/Fast LLM 从 heyi-bj 迁到 eight (`100.76.3.59`)。
...
|---|---|---|---|---|
| **MAIN（主，复杂任务）** | **Yunwu** | `MiniMax-M2.7` | `https://yunwu.ai/v1` | `YUNWU_OPEN_KEY=<填入本机密钥>` |
| 备用 1（限速时切） | Yunwu | `GLM-4.6` | 同上 | 同 key |
| 备用 2（限速时切） | Yunwu | `Kimi-K2.6` | 同上 | 同 key |
| **FAST（路由/extract/快速回复）** | eight local vLLM | `qwen3.5-9b-local-gpu0` | `http://100.76.3.59:7860/v1` | `LLM_LOCAL_API_KEY=EMPTY` |
...
仲裁器：用 qwen3.5-9b-local-gpu0 (FAST 通道) 对 RAG vs Web 两个结果做置信度打分（pre_classifier + fabrication_guard，复用 echo backend `app/web_arbitration.py` 思路）。
...
| 主机 | tailnet IP | 角色 |
|---|---|---|
| **eight** | `100.76.3.59` | STT/TTS GPU 容器 + qwen3.5-9b-local-gpu0 vLLM + ECAPA cache |
...
**Tailscale 必须开**：从 Mac 本地访问 `100.76.3.59` 的 STT/TTS/Qwen 都要走 tailnet。dev 时确保 `tailscale status` 显示 eight online。
...
| eight :7860 Qwen | → 直接走 MAIN（M2.7）做 fast tasks（成本高但兜底） |
