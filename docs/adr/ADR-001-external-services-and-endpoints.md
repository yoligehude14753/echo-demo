...
| **FAST（路由/extract/快速回复）** | eight local vLLM | `qwen3.5-9b-local` | `http://100.76.3.59:7860/v1` | `LLM_LOCAL_API_KEY=EMPTY` |
...
仲裁器：用 qwen3.5-9b-local (FAST 通道) 对 RAG vs Web 两个结果做置信度打分（pre_classifier + fabrication_guard，复用 echo backend `app/web_arbitration.py` 思路）。
...
| 主机 | tailnet IP | 角色 |
|---|---|---|
| **eight** | `100.76.3.59` | STT/TTS GPU 容器 + qwen3.5-9b-local vLLM + ECAPA cache |
