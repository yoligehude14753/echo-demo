...
`https://echodesk.yoliyoli.uk`，模型服务和密钥留在服务端。

应用内也可以在「设置 → 更新」里主动检查 GitHub Release。桌面端会在启动后自动检查
新版本；用户点击「下载并安装」后才会退出覆盖安装，本机数据目录会保留。
Android / TV 因系统限制，会打开对应 APK 下载并由系统安装器确认。

桌面端首次使用前先运行 install 脚本；之后双击 EchoDesk 即会自动启动本机 FastAPI 服务。
public demo 发布入口需要显式设置 `ECHO_PUBLIC_DEMO=1`。`ECHO_FORCE_LOCAL_BACKEND=1`
继续作为旧部署兼容开关，可覆盖 public demo 设置。
...
下面目录是 Desktop Pro 桌面端的默认数据目录。显式 public demo 模式连接
`https://echodesk.yoliyoli.uk`，客户端本地只保存 UI 配置、设备标识和临时缓存；
...
Desktop Pro 模式下，业务数据保存在本机 `~/.echodesk/`，要备份可 tar 这个目录
