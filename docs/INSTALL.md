...
> v0.2.47 macOS 资产已发布到 GitHub Release；本地开发仍可用 `npm run app:build` 生成 macOS `.app`。
...
| Android TV / 智能电视 | `EchoDesk-0.2.47-smart-tv.apk` | 适配遥控器和电视桌面入口 |
...
- macOS: `EchoDesk-0.2.47-arm64.dmg`
- Windows 安装器: `EchoDesk.Setup.0.2.47.exe`
- Windows 便携包（受管/远程环境推荐）: `EchoDesk-0.2.47-win-x64.zip`
- Linux: `EchoDesk-0.2.47.AppImage` 或 `echodesk-desktop_0.2.47_amd64.deb`
- Android 手机 / 平板: `EchoDesk-0.2.47-android.apk`（demo 包）
- Android TV / 智能电视: `EchoDesk-0.2.47-smart-tv.apk`
- 智能电视一键安装包：`EchoDesk-0.2.47-smart-tv-oneclick.zip`
...
```text
desktop/release/EchoDesk-0.2.47-arm64.dmg
desktop/release/EchoDesk-0.2.47-arm64-mac.zip
...
`EchoDesk-0.2.47-win-x64.zip`。解压后直接运行 `EchoDesk.exe`，本机数据仍保存在
...
```text
desktop/release/EchoDesk-0.2.47.AppImage
desktop/release/echodesk-desktop_0.2.47_amd64.deb
```
...
desktop/release/EchoDesk-0.2.47-android.apk
desktop/release/EchoDesk-0.2.47-android-tv.apk
desktop/release/EchoDesk-0.2.47-smart-tv.apk
desktop/release/EchoDesk-0.2.47-smart-tv-oneclick.zip
```
...
国产 Android 或 AOSP TV，可以直接安装 `EchoDesk-0.2.47-smart-tv.apk`。

推荐路径：

1. 下载 `EchoDesk-0.2.47-smart-tv-oneclick.zip`。
