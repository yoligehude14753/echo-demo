...
当前版本：`v0.2.9`
下载页：<https://github.com/yoligehude14753/echo-demo/releases/tag/v0.2.9>

| 平台 | 直接下载 | 说明 |
|---|---|---|
| macOS Apple Silicon | `EchoDesk-0.2.9-arm64.dmg` | 桌面版安装包 |
| macOS 备用 zip | `EchoDesk-0.2.9-arm64-mac.zip` | dmg 打不开时使用 |
| Windows | `EchoDesk.Setup.0.2.9.exe` | Windows 安装包 |
| Android 手机 / 平板 | `EchoDesk-0.2.9-android.apk` | 默认连接公网 demo backend |
| Android TV / 智能电视 | `EchoDesk-0.2.9-smart-tv.apk` | 适配遥控器和电视桌面入口 |
| 智能电视一键安装 | `EchoDesk-0.2.9-smart-tv-oneclick.zip` | 内含 macOS / Windows ADB 安装脚本 |
| 校验文件 | `SHA256SUMS-0.2.9.txt` | 校验下载完整性 |
...
- macOS: `EchoDesk-0.2.9-arm64.dmg`
- Windows: `EchoDesk.Setup.0.2.9.exe`
- Android 手机 / 平板: `EchoDesk-0.2.9-android.apk`（demo 包）
- Android TV / 智能电视: `EchoDesk-0.2.9-smart-tv.apk`
- 智能电视一键安装包：`EchoDesk-0.2.9-smart-tv-oneclick.zip`
...
```text
desktop/release/EchoDesk-0.2.9-arm64.dmg
desktop/release/EchoDesk-0.2.9-arm64-mac.zip
...
desktop/release/EchoDesk-0.2.9-android-tv-debug.apk
desktop/release/EchoDesk-0.2.9-smart-tv.apk
desktop/release/EchoDesk-0.2.9-smart-tv-oneclick.zip
```
...
国产 Android 或 AOSP TV，可以直接安装 `EchoDesk-0.2.9-smart-tv.apk`。

推荐路径：

1. 下载 `EchoDesk-0.2.9-smart-tv-oneclick.zip`。
2. 电视打开开发者模式和 ADB 网络调试。
3. 电脑和电视在同一个局域网。
4. macOS 执行 `./install-tv-macos.sh 电视IP`；Windows 执行
   `install-tv-windows.ps1 -TvIp 电视IP`。
5. 脚本会默认清理旧 WebView / app data、授权麦克风并尝试自动打开 EchoDesk。
   如需保留旧配置更新，运行前设置 `ECHODESK_TV_KEEP_DATA=1`。
