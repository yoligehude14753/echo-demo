...
> v0.2.44 资产需发布 workflow 生成后才会出现在 GitHub Release 中；本地开发可先用 `npm run app:build` 生成 macOS `.app`。

| 平台 | Release 资产命名 | 说明 |
|---|---|---|
| macOS Apple Silicon | `EchoDesk-0.2.44-arm64.dmg` | 桌面版安装包 |
| macOS 备用 zip | `EchoDesk-0.2.44-arm64-mac.zip` | dmg 打不开时使用 |
| Windows 安装器 | `EchoDesk.Setup.0.2.44.exe` | 普通 Windows 安装包 |
| Windows 便携包 | `EchoDesk-0.2.44-win-x64.zip` | 解压后运行 `EchoDesk.exe`；受管/远程环境优先用这个 |
| Linux AppImage | `EchoDesk-0.2.44.AppImage` | Linux x64 免安装运行 |
| Linux deb | `echodesk-desktop_0.2.44_amd64.deb` | Ubuntu / Debian 安装包 |
| Android 手机 / 平板 | `EchoDesk-0.2.44-android.apk` | 默认连接 EchoDesk 公共演示服务 |
| Android TV / 智能电视 | `EchoDesk-0.2.44-smart-tv.apk` | 适配遥控器和电视桌面入口 |
| 智能电视一键安装 | `EchoDesk-0.2.44-smart-tv-oneclick.zip` | 内含 macOS / Windows ADB 安装脚本 |
| 校验文件 | `SHA256SUMS-0.2.44.txt` | 校验下载完整性 |
...
- macOS: `EchoDesk-0.2.44-arm64.dmg`
- Windows 安装器: `EchoDesk.Setup.0.2.44.exe`
- Windows 便携包（受管/远程环境推荐）: `EchoDesk-0.2.44-win-x64.zip`
- Linux: `EchoDesk-0.2.44.AppImage` 或 `echodesk-desktop_0.2.44_amd64.deb`
- Android 手机 / 平板: `EchoDesk-0.2.44-android.apk`（demo 包）
- Android TV / 智能电视: `EchoDesk-0.2.44-smart-tv.apk`
- 智能电视一键安装包：`EchoDesk-0.2.44-smart-tv-oneclick.zip`
...
```text
desktop/release/EchoDesk-0.2.44-arm64.dmg
desktop/release/EchoDesk-0.2.44-arm64-mac.zip
...
`EchoDesk-0.2.44-win-x64.zip`。解压后直接运行 `EchoDesk.exe`，本机数据仍保存在
...
```text
desktop/release/EchoDesk-0.2.44.AppImage
desktop/release/echodesk-desktop_0.2.44_amd64.deb
```
...
desktop/release/EchoDesk-0.2.44-android.apk
desktop/release/EchoDesk-0.2.44-android-tv.apk
desktop/release/EchoDesk-0.2.44-smart-tv.apk
desktop/release/EchoDesk-0.2.44-smart-tv-oneclick.zip
```
...
国产 Android 或 AOSP TV，可以直接安装 `EchoDesk-0.2.44-smart-tv.apk`。

推荐路径：

1. 下载 `EchoDesk-0.2.44-smart-tv-oneclick.zip`。
