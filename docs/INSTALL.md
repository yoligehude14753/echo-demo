...
当前版本：`v0.2.27`
下载页：<https://github.com/yoligehude14753/echo-demo/releases/tag/v0.2.27>

| 平台 | 直接下载 | 说明 |
|---|---|---|
| macOS Apple Silicon | `EchoDesk-0.2.27-arm64.dmg` | 桌面版安装包 |
| macOS 备用 zip | `EchoDesk-0.2.27-arm64-mac.zip` | dmg 打不开时使用 |
| Windows | `EchoDesk.Setup.0.2.27.exe` | Windows 安装包 |
| Windows 便携包 | `EchoDesk-0.2.27-win-x64.zip` | 解压后运行 `EchoDesk.exe`，适合远程/托管环境 |
| Linux AppImage | `EchoDesk-0.2.27.AppImage` | Linux x64 免安装运行 |
| Linux deb | `echodesk-desktop_0.2.27_amd64.deb` | Ubuntu / Debian 安装包 |
| Android 手机 / 平板 | `EchoDesk-0.2.27-android.apk` | 默认连接公网 demo backend |
| Android TV / 智能电视 | `EchoDesk-0.2.27-smart-tv.apk` | 适配遥控器和电视桌面入口 |
| 智能电视一键安装 | `EchoDesk-0.2.27-smart-tv-oneclick.zip` | 内含 macOS / Windows ADB 安装脚本 |
| 校验文件 | `SHA256SUMS-0.2.27.txt` | 校验下载完整性 |

EchoDesk demo 现在是多端客户端 + 公网 demo backend。macOS / Windows / Linux 是桌面端；
Android / TV 是客户端壳。公开安装包默认连接 `https://echodesk.yoliyoli.uk`，
模型服务和密钥都留在服务端。

应用内也可以在「设置 → 更新」里主动检查 GitHub Release。桌面端从 `0.2.27`
...
- macOS: `EchoDesk-0.2.27-arm64.dmg`
- Windows: `EchoDesk.Setup.0.2.27.exe`
- Windows 便携包: `EchoDesk-0.2.27-win-x64.zip`
- Linux: `EchoDesk-0.2.27.AppImage` 或 `echodesk-desktop_0.2.27_amd64.deb`
- Android 手机 / 平板: `EchoDesk-0.2.27-android.apk`（demo 包）
- Android TV / 智能电视: `EchoDesk-0.2.27-smart-tv.apk`
- 智能电视一键安装包：`EchoDesk-0.2.27-smart-tv-oneclick.zip`
...
```text
desktop/release/EchoDesk-0.2.27-arm64.dmg
desktop/release/EchoDesk-0.2.27-arm64-mac.zip
...
```text
desktop/release/EchoDesk-0.2.27.AppImage
desktop/release/echodesk-desktop_0.2.27_amd64.deb
```
...
desktop/release/EchoDesk-0.2.27-android.apk
desktop/release/EchoDesk-0.2.27-android-tv.apk
desktop/release/EchoDesk-0.2.27-smart-tv.apk
desktop/release/EchoDesk-0.2.27-smart-tv-oneclick.zip
```
...
国产 Android 或 AOSP TV，可以直接安装 `EchoDesk-0.2.27-smart-tv.apk`。

推荐路径：

1. 下载 `EchoDesk-0.2.27-smart-tv-oneclick.zip`。
