...
`app:dist:mac:adhoc-test` 只用于开发验证。
...
`app:dist:win:unsigned-test` 只用于 installed smoke，不能作为正式发布资产。
...
```text
EchoDesk-0.3.1-linux-x64.AppImage
EchoDesk-0.3.1-linux-x64.deb
```
...
`EchoDesk-0.3.1-android-signing-lineage.bin`。
...
当前源码证据：packaged local smoke 通过；真实安装态 GLM + AgentOS workflow `1 / 1 passed`。这些结果不自动证明其它操作系统安装包或公开部署。
