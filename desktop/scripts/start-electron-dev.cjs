...
function publicProxyTarget(env) {
  if (env.ECHODESK_BASE_URL || env.ECHO_PRINCIPAL_MODE !== "public") {
    return env.ECHODESK_BASE_URL;
  }
  return env.VITE_API_TARGET || env.ECHO_PUBLIC_BACKEND_BASE || backendConfig.roles.publicService.baseUrl;
}

function allocateSourceBackendEndpoint({
  execFile = execFileSync,
  env = process.env,
} = {}) {
  const output = execFile(
    ...
    [
      "-e",
      "const n=require('node:net');const s=n.createServer();s.listen(0,'127.0.0.1',()=>{process.stdout.write(String(s.address().port));s.close();});",
    ],
    {
  ...
  const port = Number.parseInt(String(output || "").trim(), 10);
...
function resolveSourceBackendEndpoint(env, allocate = allocateSourceBackendEndpoint) {
  return publicProxyTarget(env) || allocate({ env });
}
...
async function run() {
  ...
  env.ECHODESK_BASE_URL = resolveSourceBackendEndpoint(env);
