...
try {
  ...
  const smokePort = String(process.env.ECHODESK_SMOKE_PORT || "").trim();
  if (!/^\d+$/.test(smokePort) || Number(smokePort) < 1024 || Number(smokePort) > 65_535) {
    throw new Error("[dmg-smoke] ECHODESK_SMOKE_PORT must be injected between 1024 and 65535");
  }
  const env = {
    ...process.env,
    HOME: home,
    ...
    ECHODESK_SMOKE_PORT: smokePort,
    ECHODESK_BASE_URL: `http://127.0.0.1:${smokePort}`,
