...
if (process.platform !== "darwin") {
  throw new Error(
    "[backend-build] macOS backend must be built on an arm64 macOS runner",
  );
}
...
function validatePython(candidate) {
  ...
  if (!/^arm64\s+/i.test(output)) {
    throw new Error(
      `[backend-build] refusing non-arm64 Python ${candidate}: ${output}`,
    );
  }
  return output;
}
...
if (!python) {
  throw new Error(
    `[backend-build] no arm64 Python with PyInstaller found; searched: ${candidates.join(", ")}`,
  );
}
...
if (
  file.error ||
  file.status !== 0 ||
  !/Mach-O.*arm64/i.test(fileDescription)
) {
  throw new Error(
    `[backend-build] output is not an arm64 Mach-O executable: ${fileDescription}`,
  );
}
