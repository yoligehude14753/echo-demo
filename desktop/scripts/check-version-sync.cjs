...
const windowsInstalledSmoke = readDesktop("scripts/windows-installed-smoke.ps1");
const requiredVersionChecks = [
  "$ExpectedVersion = [string]$packageJson.version",
  "[string]$Health.version -ne $ExpectedVersion",
  "[string]$bootstrap.app_version -ne $ExpectedVersion",
  "$productVersion.StartsWith($ExpectedVersion",
];
for (const check of requiredVersionChecks) {
  if (!windowsInstalledSmoke.includes(check)) {
    fail(`Windows installed smoke is missing version check: ${check}`);
  }
}
