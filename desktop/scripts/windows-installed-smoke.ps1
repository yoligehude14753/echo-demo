...
$installDir = Join-Path $env:LOCALAPPDATA "Programs/$installerDirectoryName"
...
try {
  ...
  if (-not (Test-Path $installedBackend -PathType Leaf)) {
    throw "installed backend is missing: $installedBackend"
  }
  if (-not (Test-Path $uninstaller -PathType Leaf)) {
    throw "installed uninstaller is missing: $uninstaller"
  }
  ...
  $result = [ordered]@{
    ...
    normal_user_data_preserved = $true
  }
} catch {
  $failure = $_
