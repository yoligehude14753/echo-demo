...
// @ts-expect-error CommonJS helper is also consumed by Electron contract tests.
...
export default defineConfig(({ command }) => {
  const viteBackendTarget =
    command === "serve" ? resolveViteBackendTarget(process.env) : null;
  return {
    plugins: [
      react(),
      legacy({
