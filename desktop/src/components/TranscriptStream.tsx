...
import { visibleTranscriptPartials } from "@/transcriptPartialState";
import ExecutionTraceCard from "@/components/ExecutionTraceCard";
import { deriveProcessTraces } from "@/lib/processTrace";
...
interface DisplaySegment {
  ...
  stableKey?: string;
  partialState?: "partial" | "completed";
}
    ...
    <div className="mt-3 ml-10 max-w-[78%]" data-testid="memory-association-card">
      <div className="rounded-2xl border border-indigo-200 bg-white px-4 py-3 shadow-sm">
        ...
        <div className="mt-2 text-[14px] text-ink-800">
          {segment.memoryLabel ?? `相关历史信息（${sources.length}）`}
        </div>
...
function meetingSegmentToDisplay(
  ...
  return {
    ...
    segmentCorrelation: s.capture_correlation,
  };
}
...
export default function TranscriptStream(): JSX.Element {
  ...
  const transcriptPartials = useStore((s) => s.transcriptPartials);
  ...
  const activeMeetingId =
    meeting?.state === "in_meeting" ? currentMeetingId : null;
  ...
  const partialSegs: DisplaySegment[] = useMemo(
    () =>
      visibleTranscriptPartials(transcriptPartials, currentMeetingId).map(
        (partial) => ({
          text: partial.text,
          captured_at: partial.capturedAt,
          speaker_label: null,
          segmentCorrelation: partial.correlation,
          stableKey: partial.correlation,
          partialState: partial.state,
        }),
      ),
    [currentMeetingId, transcriptPartials],
  );

  const segs: DisplaySegment[] = useMemo(() => {
    const canonical = dedupeDisplaySegments(
      [...baseSegs, ...dialogSegs].sort((a, b) =>
        a.captured_at.localeCompare(b.captured_at),
      ),
    );
    return [...canonical, ...partialSegs].sort((a, b) =>
      a.captured_at.localeCompare(b.captured_at),
    );
  }, [baseSegs, dialogSegs, partialSegs]);

  const processTraces = useMemo(
    () => deriveProcessTraces(events, currentMeetingId),
    [currentMeetingId, events],
  );
  ...
  // WS 负责低延迟显示，持久 transcript 每 3 秒对账一次。这样单个事件漏接、
  // 聊天请求失败或 renderer 短暂卡顿都不会让进行中的逐字稿永久停在旧快照。
  useEffect(() => {
    if (!activeMeetingId) return undefined;
    let alive = true;
    let inFlight = false;
    ...
    const tick = async (): Promise<void> => {
      ...
      inFlight = true;
      try {
        const segments = await getMeetingTranscript(activeMeetingId, {
          signal: controller.signal,
        });
        if (alive && isCurrent(originGeneration) && !controller.signal.aborted) {
          useStore.getState().hydrateMeetingSegments(activeMeetingId, segments);
        }
      } catch {
        /* 下一轮继续对账；不干扰录音和聊天 */
      } finally {
  ...
  const lastSegmentText = segs[segs.length - 1]?.text ?? "";
  const lastTraceUpdatedAt = processTraces[processTraces.length - 1]?.updatedAt ?? "";
  useEffect(() => {
  ...
  }, [lastSegmentText, lastTraceUpdatedAt, processTraces.length, segs.length]);

  if (segs.length === 0 && processTraces.length === 0) {
    if (showMeetingHistory) {
    ...
    >
      <div className="echodesk-stream-list max-w-3xl mx-auto">
        {segs.map((s, idx) => {
          ...
          const isPartial = s.partialState !== undefined;
          const c = isSelf
          ...
          return (
            <div
              key={s.stableKey ?? `${s.captured_at}-${idx}`}
              ...
              data-transcript-state={s.partialState ?? "canonical"}
            >
