...
function ProbeMetaRow({
  ...
  return (
    <div className="flex items-center justify-between text-[11px]">
      ...
      <span className="font-medium text-ink-700 text-right">{modelDisplayName(model)}</span>
    </div>
  );
}
...
function AiEnginePopover({
  ...
  const fastModel = probe(remote, "fast_model");
  ...
  return (
    <div className="min-w-[330px] max-w-[460px] text-[12px] py-1">
      ...
      <ProbeMetaRow name="主模型" model={mainModel?.model} />
      <ProbeRow name="主模型连接" probe={mainModel} />
      <ProbeMetaRow name="快速模型" model={fastModel?.model} />
...
export default function StatusBar({
  ...
  const aiEngineLevel = useMemo(() => {
    const mainModel = probe(healthz?.remote, "main_model");
    const fastModel = probe(healthz?.remote, "fast_model");
    const speechRecognition = probe(healthz?.remote, "speech_recognition");
