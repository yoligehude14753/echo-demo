...
export default function IdentityStatus(): JSX.Element | null {
  ...
  const enrollmentActionLock = useRef(false);
  ...
  if (session.phase === "error") {
    ...
    const identityMissing = session.errorCode === "IDENTITY_STORE_MISSING";
    const httpsRequired = session.message?.includes("HTTPS");
    const confirmEnrollment = () => {
      if (enrollmentActionLock.current) return;
      enrollmentActionLock.current = true;
      Modal.confirm({
        title: "创建安全设备身份？",
        content: "这会为当前 EchoDesk 与当前 HTTPS 服务建立持久设备身份。",
        okText: "创建并配对",
        cancelText: "取消",
        onOk: async () => {
          if (retrying) return;
          setRetrying(true);
          try {
            await enrollServerIdentity();
          } catch {
            // Session status already carries a sanitized failure state.
          } finally {
            setRetrying(false);
            enrollmentActionLock.current = false;
          }
        },
        onCancel: () => {
          enrollmentActionLock.current = false;
    ...
    if (identityMissing) {
      return (
        <Tooltip title={session.message ?? "尚未建立安全设备身份；请明确创建身份并重新配对。"}>
          <button
            type="button"
            className="identity-status is-error"
            data-testid="enroll-device-identity"
            data-retrying={retrying ? "true" : "false"}
            onClick={confirmEnrollment}
            disabled={retrying}
            aria-label="创建安全身份并重新配对"
          >
            <ShieldAlert aria-hidden="true" />
            <span>创建安全身份 / 重新配对</span>
          </button>
        </Tooltip>
      );
    }
