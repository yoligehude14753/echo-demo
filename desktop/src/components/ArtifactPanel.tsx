        ...
        {artifacts.length > 0 && (
          <ArtifactSection title="已生成文件" count={artifacts.length}>
            {artifacts.map((a) => {
              ...
              const sourceLink = (a.links ?? []).find(
                (link) =>
                  typeof link.meeting_id === "string" && link.meeting_id.length > 0,
              );
              const sourceMeetingId =
                typeof sourceLink?.meeting_id === "string"
                  ? sourceLink.meeting_id
                  : null;
              const sourceMeeting = sourceMeetingId ? meetings[sourceMeetingId] : undefined;
              return (
                <div
                  key={a.artifact_id}
                ...
                >
                  ...
                  {sourceMeetingId && (
                    <button
                      type="button"
                      className="mt-1 max-w-full truncate pl-1 text-left text-[11px] text-accent hover:underline"
                      aria-label={`查看来源会议${sourceMeeting ? meetingDisplayTitle(sourceMeeting) : sourceMeetingId}`}
                      onClick={(event) => {
                        event.stopPropagation();
                        selectMeeting(sourceMeetingId);
                      }}
                    >
                      来源会议：{sourceMeeting ? meetingDisplayTitle(sourceMeeting) : sourceMeetingId}
                    </button>
                  )}
                </div>
              );
...
function AgentTaskCardView({
  ...
  const errorText =
    typeof snapshot.error === "string" && snapshot.error.trim().length > 0
      ...
      : task.error?.trim() ?? "";
  const taskModel =
    typeof snapshot.model === "string" && snapshot.model.trim().length > 0
      ? snapshot.model.trim()
      : null;
  const taskSteps = Array.isArray(snapshot.steps)
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap items-center gap-1.5 text-[10px] text-ink-400">
        <span className="rounded border border-paper-300 bg-paper-100 px-1.5 py-0.5">
          模型：
          <strong className="ml-0.5 font-medium text-ink-700" data-testid="agent-task-model">
            {taskModel ?? "未确认"}
          </strong>
        </span>
        {task.route && (
      ...
      {taskSteps.length > 0 && (
        <ol className="space-y-1 border-l border-paper-300 pl-3 text-[10px] text-ink-500" data-testid="agent-task-steps">
