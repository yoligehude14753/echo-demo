...
const DOORS: DoorRow[] = [
  ...
  { key: "gated_stationary_noise", label: "已过滤稳定底噪", tone: "neutral" },
...
function primaryOperationalLabel(status: CaptureViewModel): string {
  if (status.transport.recovering) {
    return "正在恢复待发送片段";
  }
  ...
  if (status.admission.warning === "stationary_noise") {
    return "已过滤稳定底噪";
  }
  ...
  return (
    ...
    >
      ...
      <div className="space-y-1 text-slate-600">
        ...
        <div className="flex justify-between gap-3">
          ...
          <span className="tabular-nums">
            确认 {transport.acknowledged} · 尝试 {transport.sent} · 最近成功 {formatEpochRelative(transport.lastSuccessfulUploadAt)}
          </span>
        </div>
      ...
      {transport.warning === "backpressure" && (
        <div className="text-amber-700">有界发送队列正在排空；超限或过期片段会明确计数。</div>
      )}
      ...
      {admission.warning === "stationary_noise" && (
        <div className="text-amber-700">当前输入只有稳定能量，没有检测到语音音节变化，已停止送入识别模型。</div>
      )}
...
export default function CaptureStatus({ status }: Props): JSX.Element {
  ...
  if (runtimeState === "capture_stopped") {
    return (
      <Tag color="red" data-testid="capture-status" tabIndex={-1}>
        麦克风已停止
      </Tag>
    ...
    if (transport.recovering) {
      return (
        <Tag
          ...
          tabIndex={-1}
        >
          恢复待发送 {transport.queueDepth}/{transport.queueCapacity}
        </Tag>
      );
    }
  ...
  return (
    ...
    >
      <Tag
        ...
        data-recovering={transport.recovering ? "1" : "0"}
