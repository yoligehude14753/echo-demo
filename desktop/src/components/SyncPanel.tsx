...
export default function SyncPanel(): JSX.Element {
  ...
  const displayState: SyncState = {
    ...state,
    status: projectSyncStatus(state, hostPaired),
  };
  ...
  const pair = async () => {
    ...
    try {
      ...
      setState(loadSyncState());
      setPairingCode("");
      message.success("设备已配对，正在校验全量历史");
    } catch (error) {
  ...
  const initializeCurrentDevice = async () => {
    ...
    try {
      ...
      setState(loadSyncState());
      setPairingCode("");
      message.success("本机同步已初始化，历史将开始上传");
    } catch (error) {
      const detail = syncErrorMessage(error);
      updateSyncState((current) => ({ ...current, status: "failed", last_error: detail }));
      setState(loadSyncState());
      ...
      >
        <div className="space-y-4 py-2 text-[12px] text-ink-600">
          ...
          <div className="flex justify-end gap-2">
            ...
            {!state.sync_token && (
              <Button
                onClick={() => void initializeCurrentDevice()}
                loading={busy}
                data-testid="sync-initialize-current-device"
              >
                初始化本机同步
              </Button>
            )}
