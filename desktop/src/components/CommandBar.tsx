 ...
 * 行为：
 * 1. 用户输入文本，回车提交
 * 2. 明确的产物命令直接调 POST /artifacts/generate；自由文本才调 /intent/route
 * 3. 按 kind 分发：
...
import type { AnswerSource, IntentKind, IntentResult } from "@/types";
    ...
    switch (r.kind) {
      ...
      case "chat_no_rag": {
        ...
        void chatAsk(question, {
          conversationId,
          messageId,
          meetingId: interactionMeetingId,
      ...
      default: {
        ...
        void ragAsk(question, {
          ...
          meetingId: interactionMeetingId,
          onMemoryFrame: emitMemoryFrame,
          answerSources: Array.isArray(r.params.answer_sources)
            ? r.params.answer_sources.filter(
