...
import { requestFormalMeeting } from "@/components/meetingStartLifecycle";
...
export default function MinutesView(): JSX.Element {
  ...
  useEffect(() => {
    if (!currentId) return;
    // local-only 卡片由 localStorage / legacy IPC 提供详情；其 opaque id 不能拿去
    // 请求公网 minutes/workflow 端点。
    if (shouldHideSharedPublicHistory()) return;
    if (meeting?.minutes) return;
    if (
      meeting?.minutes_status === "generation_failed" ||
      meeting?.minutes_status === "no_content"
    ) return;
    ...
    const scheduleRetry = () => {
      if (!canCommit()) return;
      retryTimer = setTimeout(() => void loadMinutes(), 2_000);
    };
    const loadMinutes = async (): Promise<void> => {
      if (!canCommit()) return;
      try {
        const [m, workflowRuns] = await Promise.all([
        ...
        if (!canCommit()) return;
        if (!m) {
          // /minutes 的 404 同时可能表示“仍在生成”或“生成已失败”。WS
          // 断线时必须回读 durable meeting status，不能把两种状态都无限
          // 投影成 spinner。
          const summary = (await listMeetings(50, {
            signal: controller.signal,
          })).find((item) => item.meeting_id === currentId);
          if (!canCommit()) return;
          if (summary?.minutes_status === "generation_failed") {
            upsertMeeting(currentId, {
              state: "ended",
              ended_at: summary.ended_at ?? meeting?.ended_at,
              minutes_status: "generation_failed",
              minutes_error: summary.minutes_error,
            });
            return;
          }
          if (summary?.minutes_status === "no_content") {
            upsertMeeting(currentId, {
              state: "ended",
              ended_at: summary.ended_at ?? meeting?.ended_at,
  ...
  } else if (meeting?.minutes_status === "no_content") {
    activeMinutes = (
      <div className="minutes-current-empty px-6 py-6">
        <div className="flex items-center gap-2 mb-4 text-[13px] text-ink-700 font-medium">
          ...
          <span>会议纪要</span>
          <span className="ml-auto">{shareAction}</span>
        </div>
        <Empty
              ...
              {inMeeting ? (
                ...
                <span className="flex flex-col items-center gap-2">
                  ...
                  <Button
                    ...
                    onClick={requestFormalMeeting}
                    data-testid="start-formal-meeting-from-minutes"
                  >
                    开始正式会议
                  </Button>
                </span>
...
function MinutesBody({
  ...
  return (
    <div className="minutes-current-content px-6 py-5 border-b border-paper-300">
