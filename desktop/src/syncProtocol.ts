import {
  ...
  normalizeSyncCursor,
  setPairingState,
  SYNC_HISTORY_ENTITY_TYPES,
  type SyncEntityType,
  type SyncHistoryEntityType,
...
export interface SyncSnapshotResponse {
  ...
  changes?: SyncChange[];
  meetings?: SyncChange[];
  transcript_segments?: SyncChange[];
  meeting_summaries?: SyncChange[];
  artifacts?: SyncChange[];
  memories?: SyncChange[];
}
...
function normalizeHistoryManifest(value: unknown): SyncHistoryManifest | null {
  ...
  if (typeof value !== "object" || Array.isArray(value)) {
    throw new SyncApiError("同步完整性清单格式无效");
  }
  const candidate = value as {
    cursor?: unknown;
    entity_counts?: Partial<Record<SyncHistoryEntityType, unknown>>;
  };
  const cursor = normalizeChangesCursor(candidate.cursor);
  if (cursor === null || !candidate.entity_counts || typeof candidate.entity_counts !== "object") {
    throw new SyncApiError("同步完整性清单缺少游标或实体计数");
  }
  const entityCounts = {} as Record<SyncHistoryEntityType, number>;
  for (const entityType of SYNC_HISTORY_ENTITY_TYPES) {
    const count = candidate.entity_counts[entityType];
    if (!Number.isSafeInteger(count) || Number(count) < 0) {
      throw new SyncApiError("同步完整性清单包含无效的实体计数");
    }
    entityCounts[entityType] = Number(count);
  }
...
export function normalizePairingCreateResponse(value: unknown): PairingCreateResponse {
  const response = value as { pairing_code?: unknown; expires_at?: unknown } | null;
  const pairingCode = response?.pairing_code;
  if (typeof pairingCode !== "string" || !pairingCode.trim()) {
    throw new SyncApiError("配对响应缺少有效的配对码");
  }
  const expiresAt = response?.expires_at;
  return {
    pairing_code: pairingCode.trim(),
    expires_at: typeof expiresAt === "string" && expiresAt.trim() ? expiresAt : null,
  };
}

function normalizeChanges(
  value: SyncChangesResponse | SyncSnapshotResponse | SyncChange[],
): SyncChangesResponse {
  const response = Array.isArray(value) ? null : value;
  const snapshot =
    response &&
    !Array.isArray(response.changes) &&
    ("meetings" in response || "transcript_segments" in response)
      ? response as SyncSnapshotResponse
      : null;
  const changes = Array.isArray(value)
    ...
    : Array.isArray(response?.changes)
      ...
      : [
          ...(snapshot?.meetings ?? []),
          ...(snapshot?.transcript_segments ?? []),
          ...(snapshot?.meeting_summaries ?? []),
          ...(snapshot?.artifacts ?? []),
          ...(snapshot?.memories ?? []),
        ];
  ...
  return {
    ...
    cursor: Array.isArray(value) ? null : normalizeChangesCursor(response?.cursor),
    manifest: Array.isArray(value) ? null : normalizeHistoryManifest(response?.manifest),
    reset_required:
      Array.isArray(value) || snapshot ? false : (response as SyncChangesResponse).reset_required === true,
    snapshot_required:
      Array.isArray(value) || snapshot ? false : (response as SyncChangesResponse).snapshot_required === true,
  };
}
...
export class SyncHubClient {
  ...
  async createPairing(): Promise<PairingCreateResponse> {
    ...
    return normalizePairingCreateResponse(await readJson<unknown>(response));
  }

  async initializeCurrentDevice(): Promise<PairingClaimResponse> {
    const pairing = await this.createPairing();
    return this.claimPairing(pairing.pairing_code);
  }
  ...
  async snapshot(): Promise<SyncChangesResponse> {
    requireSyncToken(this.state());
    const response = await this.transport.request(
