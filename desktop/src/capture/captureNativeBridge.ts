...
export function normalizeNativeCaptureUpload(
  value: unknown,
  expected: { deviceId: string; captureSessionId: string; source: "desktop" },
): NativeCaptureUploadResult | null {
  const body = record(value);
  if (
    body.deviceId !== expected.deviceId ||
    body.captureSessionId !== expected.captureSessionId ||
    body.source !== expected.source
  ) {
    return null;
  }
