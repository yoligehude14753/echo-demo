...
test("native success payload maps to the same renderer correlation as recent", () => {
  ...
  const native = normalizeNativeCaptureUpload({
    ...
    ambientStored: true,
    ambientText: "同一段转写",
    captureSessionId: scope.captureSessionId,
...
test("native success from a replaced capture session is ignored", () => {
  const accepted = normalizeNativeCaptureUpload({
    segmentCorrelation: "seg-0123456789abcdef",
    ambientStored: true,
