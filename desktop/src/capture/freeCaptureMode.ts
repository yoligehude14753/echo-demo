...
export function deriveCaptureRuntimeState(
  ...
  if (input.captureState === "standby") return "capture_stopped";
  if (input.captureState === "initializing") return "free_starting";
...
export function installFreeCaptureCommandBridge(
  onCommand?: (command: FreeCaptureCommand) => void,
): () => void {
