...
export type CaptureControlAuthority = "backend" | "local";
...
export interface CaptureControl {
  ...
  revision: number;
  /** Local is renderer-only and is never accepted from a backend response. */
  authority?: CaptureControlAuthority;
}
...
export function normalizeCaptureControl(
  value: unknown,
  authority: CaptureControlAuthority = "backend",
): CaptureControl {
