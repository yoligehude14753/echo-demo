...
test("standby is a stopped microphone state, not an initializing state", () => {
  assert.equal(
    deriveCaptureRuntimeState({ ...base, captureState: "standby" }),
    "capture_stopped",
  );
});

test("same control only deduplicates while capture is already active", () => {
  const hook = readFileSync(
    new URL("./useEchoCapture.ts", import.meta.url),
    "utf8",
  );
  assert.match(hook, /isCaptureLifecycleActive\(audioCapture\.getState\(\)\)/);
});
...
test("pausing capture keeps the durable upload router available for drain", () => {
  const hook = readFileSync(
    new URL("./useEchoCapture.ts", import.meta.url),
    "utf8",
  );
  ...
  assert.match(
    hook,
    /if \(!captureEnabled\) \{[\s\S]{0,180}audioCapture\.stop\(\);[\s\S]{0,180}return;/,
  );
  assert.match(hook, /const offRouter = attachCaptureChunkRouter\(/);
  assert.match(hook, /captureEnabled,\n {4}isCurrent/);
});
...
test("formal meeting component delegates lifecycle instead of waiting for capture", () => {
  const status = readFileSync(
    new URL("../components/MeetingStatusBar.tsx", import.meta.url),
    "utf8",
  );
  assert.doesNotMatch(status, /await ensureServerSession\(\)/);
  assert.match(status, /resolveCaptureSetupPolicy/);
  assert.match(status, /authority: "local" as const/);
  assert.match(status, /const control = await updateCaptureControl/);
  assert.match(status, /announceCaptureControl\(control\);/);
  assert.doesNotMatch(status, /waitForFirstFrame/);
  assert.match(status, /startFormalMeetingLifecycle\(\{/);
  assert.match(status, /manualStart: manualStartMeeting/);
  assert.match(status, /endFormalMeetingLifecycle\(\{/);
  assert.match(status, /manualEnd: manualEndMeeting/);
  assert.match(status, /prepareCapture: prepareCaptureAfterFormalStart/);
  assert.doesNotMatch(
    status,
    /\b(?:startMeeting|endMeeting|finalizeMeeting|newLocalMeetingId|isLocalOnlyMeetingId|shouldHideSharedPublicHistory)\b/,
  );
