...
test("paired Hub transport rejects ordinary product API paths", () => {
  assert.doesNotThrow(() => assertPairedHubPath("/hub/v1/pairings"));
  assert.doesNotThrow(() => assertPairedHubPath("/hub/v1/sync/changes?cursor=0"));
  assert.throws(() => assertPairedHubPath("/meetings"), /仅允许/);
});
