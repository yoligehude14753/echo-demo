...
// The backend owns a 30s ASR deadline and must still have time to commit the
// failed terminal receipt.  Aborting at 20s leaves the durable claim in
// processing and turns an ASR timeout into capture backpressure.
const CAPTURE_UPLOAD_TIMEOUT_MS = 40_000;
...
export type CaptureChunkAdmissionStatus =
  | "accepted"
  | "pending"
  | "rejected"
  | "unknown";

export interface CaptureChunkAdmission {
  /** Restricted backend status; unknown values are never admission. */
  status: CaptureChunkAdmissionStatus;
  /** Only an explicit backend admission may authorize local acknowledgement. */
  accepted: boolean;
  /** The backend has durably admitted the audio and can replay it by key. */
  durable: boolean;
  /** Opaque backend receipt identifier; raw hashes never enter client state. */
...
export interface CaptureChunkResponse {
  ...
  /** Echoed capture tuple; absent values deliberately fail UI admission. */
  device_id: string | null;
  capture_session_id: string | null;
  source: string | null;
  /** Safe projection of the backend durable-admission receipt. */
  admission: CaptureChunkAdmission;
}
...
export class CaptureUploadHttpError extends Error {
  constructor(
    ...
    public readonly errorClass: string | null = null,
  ) {
    super(`EchoDesk 收音上传失败（HTTP ${status}）`);
    this.name = "CaptureUploadHttpError";
  }
}

function captureRetryAfterMs(response: Response): number | null {
...
const CAPTURE_ADMISSION_STATUSES: readonly CaptureChunkAdmissionStatus[] = [
  "accepted",
  "pending",
  "rejected",
  "unknown",
...
function normalizeCaptureAdmissionStatus(
  value: unknown,
): CaptureChunkAdmissionStatus {
  return CAPTURE_ADMISSION_STATUSES.includes(
    value as CaptureChunkAdmissionStatus,
  )
    ? (value as CaptureChunkAdmissionStatus)
    : "unknown";
}

function nullableCaptureReceiptId(value: unknown): string | null {
...
export function normalizeCaptureChunkResponse(
  ...
  // Never bind an unscoped HTTP response to the request scope. The request
  // scope is only an input to the upload; acknowledgement requires the
  // backend to echo the response attribution explicitly.
  void requestScope;
  const attribution = resolveCaptureResponseAttribution(body);
  const admission = captureRecord(body.admission);
  return {
    ...
    capture_mode:
      body.capture_mode === "formal" || body.capture_mode === "auto"
        ? body.capture_mode
        : "free",
    device_id: nullableString(attribution.device_id),
    capture_session_id: nullableString(attribution.capture_session_id),
    source: nullableString(attribution.source),
    admission: {
      status: normalizeCaptureAdmissionStatus(admission.status),
      accepted: admission.status === "accepted",
      durable: admission.durable === true,
      receipt_id: nullableCaptureReceiptId(admission.receipt_id),
    },
  };
}

export function isDurableCaptureChunkReceipt(
  response: CaptureChunkResponse,
  requestScope: Pick<CaptureScope, "deviceId" | "source"> & {
    captureSessionId?: string;
  },
): boolean {
  return (
    response.admission.status === "accepted" &&
    response.admission.accepted === true &&
    response.admission.durable === true &&
    response.admission.receipt_id !== null &&
    response.device_id === requestScope.deviceId &&
    response.capture_session_id === requestScope.captureSessionId &&
    response.source === requestScope.source
  );
}
...
export async function uploadCaptureChunk(
  ...
  const deviceId = options.scope?.deviceId ?? options.deviceId ?? captureDeviceId();
  const segmentId =
    options.segmentId ??
  ...
  if (options.capturedAtMs !== undefined) {
    fd.append("captured_at_ms", String(options.capturedAtMs));
  }
  if (options.scope) {
    fd.append("captureSessionId", options.scope.captureSessionId);
    fd.append("source", options.scope.source);
  }
  ...
  if (
    ...
    new URL(u, window.location.href).origin !== options.expectedOrigin
  ) {
    throw new Error("收音上传目标已切换");
  }
  ...
  if (!r.ok) {
    const retryAfterMs = captureRetryAfterMs(r);
    const errorClass = r.headers.get("X-Capture-Error-Class");
    await r.body?.cancel().catch(() => undefined);
    throw new CaptureUploadHttpError(r.status, retryAfterMs, errorClass);
  }
  const parsed = await asJson<unknown>(r);
  return normalizeCaptureChunkResponse(parsed, options.scope);
}
...
export function normalizeCaptureStats(value: unknown): CaptureStats {
  ...
  return {
    ...
    gated_stationary_noise: finiteNonNegative(body.gated_stationary_noise),
...
export interface RagAskOptions {
  ...
  messageId?: string;
  /** Explicit current-meeting scope; Qwen still reads and selects original chunks. */
  meetingId?: string;
  onMemoryFrame?: (frame: MemoryFramePayload) => void;
  answerSources?: import("@/types").AnswerSource[];
}
  ...
  const r = await fetchWithAbortTimeout(
    u,
    {
      ...
      body: JSON.stringify({
        ...
        answer_sources: options.answerSources,
      }),
    },
  ...
  try {
    for (;;) {
      ...
      while ((nl = buf.indexOf("\n\n")) >= 0) {
        ...
        if (typeof obj.model_id === "string") model = obj.model_id;
...
export async function memoryRecall(
  ...
  return {
    ...
    label: sources.length > 0 ? `相关历史信息（${sources.length}）` : "",
    conversation_id: conversationId,
    message_id: messageId,
    ...(result.small_model
      ? { model_id: result.small_model, model_display_name: result.small_model }
      : {}),
