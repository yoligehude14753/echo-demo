...
export const BACKEND_READINESS_EVENT = "echodesk:backend-readiness";
...
export interface BackendBootstrap {
  ...
  /** Local mode projects this server-owned scope without issuing a bearer. */
...
let identityStatus: SessionIdentityStatus = { phase: "idle", message: null, errorCode: null };
let enrollmentPromise: Promise<string> | null = null;
let enrollmentPromiseOrigin: string | null = null;
...
if (typeof window !== "undefined") {
  window.addEventListener(BACKEND_ORIGIN_EVENT, () => {
    ...
    currentSession = null;
    publishBackendReadiness(INITIAL_BACKEND_READINESS, null, null);
  });
}
...
function publishIdentityStatus(
  ...
  const inferredCode =
    errorCode ??
    (normalizedMessage.includes("identity_store_missing") ||
    normalizedMessage.includes("stored device identity is required")
      ? "IDENTITY_STORE_MISSING"
      : normalizedMessage.includes("安全身份存储不可用")
        ? "IDENTITY_STORE_UNAVAILABLE"
        : null);
  identityStatus = { phase, message, errorCode: inferredCode };
...
function publishBackendReadiness(
  ...
  if (typeof window !== "undefined") {
    window.dispatchEvent(new Event(BACKEND_READINESS_EVENT));
  }
}
...
/** Capture transport may run only against the active, fully established session. */
export function backendSessionTransportReady(): boolean {
  return (
    readiness.reachability === "reachable" &&
    (readiness.auth === "not_required" || readiness.auth === "authenticated") &&
    (readiness.api_contract === "compatible" || readiness.api_contract === "legacy") &&
    currentSession !== null &&
    currentSession.origin === activeBackendOrigin &&
    identityStatus.phase === "ready"
  );
}

export function subscribeBackendSessionTransportReadiness(
  listener: (ready: boolean) => void,
): () => void {
  if (typeof window === "undefined") return () => undefined;
  let projected = backendSessionTransportReady();
  const publishProjection = (): void => {
    const next = backendSessionTransportReady();
    if (next === projected) return;
    projected = next;
    listener(next);
  };
  window.addEventListener(BACKEND_READINESS_EVENT, publishProjection);
  window.addEventListener(SESSION_IDENTITY_EVENT, publishProjection);
  window.addEventListener(BACKEND_ORIGIN_EVENT, publishProjection);
  return () => {
    window.removeEventListener(BACKEND_READINESS_EVENT, publishProjection);
    window.removeEventListener(SESSION_IDENTITY_EVENT, publishProjection);
    window.removeEventListener(BACKEND_ORIGIN_EVENT, publishProjection);
  };
}
...
function userVisibleIdentityError(error: unknown): string {
  ...
  if (
    code === "IDENTITY_STORE_MISSING" ||
    message.includes("identity_store_missing") ||
    message.includes("stored device identity is required")
  ) {
    return "尚未建立安全设备身份；请明确创建身份并重新配对";
  }
  if (
...
function errorCodeForIdentityError(error: unknown): string | null {
  const code =
    typeof error === "object" && error !== null && "code" in error
      ? String((error as { code?: unknown }).code ?? "")
      : "";
  const message = error instanceof Error ? error.message.toLowerCase() : "";
  if (code === "IDENTITY_STORE_MISSING" || message.includes("stored device identity is required")) {
    return "IDENTITY_STORE_MISSING";
  }
  if (code === "IDENTITY_STORE_UNAVAILABLE" || message.includes("secure store")) {
...
function acceptLocalBootstrapPrincipal(
  ...
  if (
    ...
    throw new BackendContractMismatchError("local-principal-missing");
  }
  const tenantId = principal.tenant_id;
  const ownerId = principal.owner_id;
  ...
  if (activeBackendOrigin === origin) {
    currentSession = {
      token: "",
      expiresAt: null,
      scopeKey: `${tenantId.trim()}:${ownerId.trim()}`,
...
/** Stable opaque partition label; raw principal scope never enters IndexedDB. */
export async function currentSessionScopeFingerprint(): Promise<string | null> {
  const session = currentSession;
  if (!session || !globalThis.crypto?.subtle) return null;
  const digest = await globalThis.crypto.subtle.digest(
    "SHA-256",
    new TextEncoder().encode(session.scopeKey),
  );
  if (
    currentSession?.scopeKey !== session.scopeKey ||
    currentSession.origin !== session.origin
  ) {
    return null;
  }
  const bytes = new Uint8Array(digest).subarray(0, 16);
  return `scope-${Array.from(bytes, (value) =>
...
function selectBackendOrigin(origin: string): void {
  ...
  publishBackendReadiness(INITIAL_BACKEND_READINESS, null, origin);
  if (typeof document !== "undefined") {
...
function publishIdentityStatusForOrigin(
  ...
  if (activeBackendOrigin === origin) {
    publishIdentityStatus(phase, message, errorCode);
  }
}
...
/**
 * User-confirmed enrollment for a device with no durable identity vault.
 * This is intentionally separate from the automatic session bootstrap path.
 */
export async function enrollServerIdentity(): Promise<string> {
  const origin = await configuredBackendOrigin();
  selectBackendOrigin(origin);
  throwIfClientUpgradeRequired(origin);
  if (enrollmentPromise && enrollmentPromiseOrigin === origin) {
    return enrollmentPromise;
  }
  currentSession = null;
  const pending = (async () => {
    publishIdentityStatus("establishing");
    try {
      const request = window.echo?.enrollPublicSession;
      if (!request) throw new Error("explicit enrollment bridge is unavailable");
      const token = acceptSession(await request(), origin);
      if (!token) throw new Error("explicit enrollment returned no session");
      publishIdentityStatusForOrigin(origin, "ready");
      return token;
    } catch (error) {
      if (activeBackendOrigin === origin) {
        publishIdentityStatusForOrigin(
          ...
          errorCodeForIdentityError(error),
        );
      }
      throw error;
    }
  })();
  enrollmentPromise = pending;
  enrollmentPromiseOrigin = origin;
  try {
    return await pending;
  } finally {
    if (enrollmentPromise === pending) {
      enrollmentPromise = null;
      enrollmentPromiseOrigin = null;
    }
  }
}
...
export async function ensureServerSession(force = false): Promise<string | null> {
  ...
  if (force) {
    ...
    const pending = (async () => {
      ...
      if (!requiresSession) {
        return acceptLocalBootstrapPrincipal(bootstrap, origin);
      }
...
export async function apiTransport(
  ...
  try {
    ...
    const actualOrigin = new URL(url, window.location.href).origin;
    const usesViteProxyOrigin =
      usesElectronViteProxy() && actualOrigin === window.location.origin;
    if (
      leaseEpoch !== transportOriginEpoch ||
      controller.signal.aborted ||
      (actualOrigin !== leaseOrigin && !usesViteProxyOrigin)
    ) {
      throw new ApiTransportError(
        "后端地址已切换，已取消旧服务请求",
        "stale-origin",
    ...
    if (response.url) {
      ...
      const usesViteProxyResponseOrigin =
        usesElectronViteProxy() && responseOrigin === window.location.origin;
      if (responseOrigin !== leaseOrigin && !usesViteProxyResponseOrigin) {
        await response.body?.cancel("backend response origin changed").catch(() => {});
        throw new ApiTransportError(
