...
function boundedJsonError(code, { status = null, cause } = {}) {
  const messages = {
    ...
    HTTPS_JSON_ABORTED: "HTTPS JSON request was aborted",
  };
...
function requestBoundedHttpsJson(
  rawUrl,
  {
    method = "GET",
    headers = {},
  ...
  let target;
  try { target = new URL(String(rawUrl || "")); } catch (cause) {
    return Promise.reject(boundedJsonError("HTTPS_JSON_URL_INVALID", { cause }));
  }
  if (target.protocol !== "https:" || target.username || target.password ||
      !Number.isSafeInteger(maxBytes) || maxBytes < 1 ||
      !Number.isSafeInteger(timeoutMs) || timeoutMs < 1 ||
      typeof requestImpl !== "function" || typeof validate !== "function") {
    return Promise.reject(boundedJsonError("HTTPS_JSON_URL_INVALID"));
  }
  const payload = body === undefined ? null : Buffer.from(String(body));
  return new Promise((resolve, reject) => {
    ...
    const onAbort = () => {
      ...
      fail(boundedJsonError("HTTPS_JSON_ABORTED", { cause: reason }));
    ...
    try {
      request = requestImpl(target, {
        ...
        if (status >= 300 && status < 400) {
          response.resume?.();
          fail(boundedJsonError("HTTPS_JSON_REDIRECT_FORBIDDEN", { status }));
          return;
        }
        if (rejectHttpErrors && (status < 200 || status >= 300)) {
          response.resume?.();
        ...
        if (contentType !== "application/json") {
          response.resume?.();
          fail(boundedJsonError("HTTPS_JSON_CONTENT_TYPE_INVALID", { status }));
          return;
        }
        try { safeContentLength(response.headers?.["content-length"], maxBytes); }
        catch (error) { response.resume?.(); fail(error); return; }
        const chunks = [];
        let receivedBytes = 0;
        response.on("data", (rawChunk) => {
          ...
          if (receivedBytes > maxBytes) {
            const error = boundedJsonError("HTTPS_JSON_RESPONSE_TOO_LARGE", { status });
            fail(error); response.destroy?.(error); return;
          }
          chunks.push(chunk);
        });
        response.on("error", (cause) => fail(boundedJsonError("HTTPS_JSON_NETWORK_ERROR", { status, cause })));
        response.on("end", () => {
          if (settled) return;
          let payloadValue;
          try { payloadValue = JSON.parse(Buffer.concat(chunks, receivedBytes).toString("utf8")); }
          catch (cause) { fail(boundedJsonError("HTTPS_JSON_RESPONSE_INVALID", { status, cause })); return; }
          let valid = false;
          try { valid = validate(payloadValue) === true; } catch { valid = false; }
          if (!valid) { fail(boundedJsonError("HTTPS_JSON_RESPONSE_INVALID", { status })); return; }
          finish(resolve, { ok: status >= 200 && status < 300, status, headers: responseHeaders(response.headers), url: target.href, body: payloadValue, json: async () => payloadValue });
        });
      });
      ...
      timer = setTimer(() => {
        ...
        fail(error); request?.destroy?.(error);
      }, timeoutMs);
      timer?.unref?.();
      if (payload !== null) request.write(payload);
      request.end();
    } catch (cause) { fail(boundedJsonError("HTTPS_JSON_NETWORK_ERROR", { cause })); }
  });
}
...
function fetchBoundedHttpsJson(
  ...
  if (getImpl === https.get) {
    return requestBoundedHttpsJson(rawUrl, { headers, maxBytes, timeoutMs, validate, setTimer, clearTimer });
  }
  let target;
  try {
