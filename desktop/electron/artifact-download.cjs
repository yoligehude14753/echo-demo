...
function downloadRendererBlob({
  ...
  const audit = (event, fields = {}) => {
    try {
      logger(JSON.stringify({ event: `artifact.download.${event}`, ...fields }));
    } catch {
      // Diagnostic logging must never change the download result.
    }
  };
  audit("requested", { suggested_filename: suggestedFilename || null });
  if (
  ...
  return new Promise((resolve, reject) => {
    ...
    const onUpdated = (_event, state) => {
      ...
      const receivedBytes = matchedItem?.getReceivedBytes();
      if (matchedItem && Number.isSafeInteger(receivedBytes) && receivedBytes > maxBytes) {
        policyCancelled = true;
        matchedItem.cancel();
      ...
      if (state === "completed") {
        try {
          ...
          if (
            !stat.isFile() ||
            stat.size > maxBytes
            || (Number.isSafeInteger(expectedBytes) && stat.size !== expectedBytes)
          ) {
            rejectCode("ARTIFACT_DOWNLOAD_INTERRUPTED");
            return;
          }
        ...
        finish(() => {
          audit("completed", { filename, size_bytes: fs.statSync(target).size });
          resolve({ ok: true, cancelled: false, filename });
        });
      } else if (state === "cancelled" && !policyCancelled) {
        finish(() => {
          audit("cancelled", { filename });
          removePartial();
    ...
    const onWillDownload = (event, item, webContents) => {
      ...
      const knownTotal = Number.isSafeInteger(totalBytes) && totalBytes >= 0;
      if (knownTotal && totalBytes > maxBytes) {
        event.preventDefault();
        rejectCode("ARTIFACT_DOWNLOAD_TOO_LARGE");
        return;
      }
      clearTimeout(startTimer);
      matchedItem = item;
      expectedBytes = knownTotal ? totalBytes : null;
      audit("started", {
        total_bytes: knownTotal ? totalBytes : null,
        size_known: knownTotal,
      });
