...
const { resolveProductDataRoot } = require("./product-data-root.cjs");
const {
...
function packagedProductName() {
  ...
  const markerIndex = executable.lastIndexOf(contentsMarker);
  if (markerIndex < 0) return null;
  const bundleRoot = executable.slice(0, markerIndex + `${path.sep}Contents`.length);
  const infoPath = path.join(bundleRoot, "Info.plist");
  try {
    const plist = fs.readFileSync(infoPath, "utf8");
    const match = plist.match(
      /<key>CFBundle(?:DisplayName|Name)<\/key>\s*<string>([^<]+)<\/string>/,
    );
...
// Source Electron 默认以 Electron 作为应用身份。打包态优先读取 bundle
// 的真实身份，确保候选包双击启动时不会落入 canonical userData 命名空间。
const desktopPackage = require("../package.json");
const DESKTOP_PRODUCT_NAME = String(
  packagedProductName() || desktopPackage.build?.productName || "EchoDesk",
);
app.setName(DESKTOP_PRODUCT_NAME);

// Package identity owns the backend data root as well as Chromium userData.
// A build-time ECHO_USER_DIR does not survive a normal Finder launch, so the
// legacy candidate must derive its persistent root at runtime.
process.env.ECHO_USER_DIR = resolveProductDataRoot({
  env: process.env,
  homeDir: os.homedir(),
  productName: DESKTOP_PRODUCT_NAME,
});

// The legacy candidate must never share Electron's lock or profile with the
// canonical application. This is intentionally before requestSingleInstanceLock.
const configuredElectronUserData = String(process.env.ECHODESK_ELECTRON_USER_DATA_DIR || "").trim();
const isolatedElectronUserData = path.resolve(
  configuredElectronUserData || path.join(app.getPath("appData"), DESKTOP_PRODUCT_NAME),
);
if (configuredElectronUserData && !path.isAbsolute(configuredElectronUserData)) {
  throw new Error("ECHODESK_ELECTRON_USER_DATA_DIR must be absolute");
}
app.setPath("userData", isolatedElectronUserData);
...
function isTrustedDevCertificateError(rawUrl, error) {
  if (!IS_DEV || !String(error || "").endsWith("ERR_CERT_AUTHORITY_INVALID")) {
    return false;
  }
  try {
    const candidate = new URL(rawUrl);
    const vite = new URL(VITE_URL);
    if (candidate.origin !== vite.origin) return false;
    return (
      vite.protocol === "https:" &&
      ["localhost", "127.0.0.1", "::1"].includes(vite.hostname) &&
      !vite.username &&
      !vite.password
    );
  } catch {
...
if (IS_DEV) {
  app.on("certificate-error", (event, _webContents, url, error, _certificate, callback) => {
    if (isTrustedDevCertificateError(url, error)) {
...
function normalizeLifecycleEndpoint(raw) {
...
function liveRuntimeRecordEndpoint(env) {
  const root = String(env.ECHO_USER_DIR || path.join(os.homedir(), ".echodesk"));
  const recordPath = path.join(path.resolve(root), "runtime", "endpoint.json");
  try {
    const record = JSON.parse(fs.readFileSync(recordPath, "utf8"));
    if (!Number.isSafeInteger(Number(record?.pid)) || !normalizeLifecycleEndpoint(record?.base_url)) {
      return null;
    }
    process.kill(Number(record.pid), 0);
    return normalizeLifecycleEndpoint(record.base_url);
  } catch {
    return null;
  }
}
function allocateLifecycleEndpoint() {
  const result = spawnSync(
    process.execPath,
    ["-e", "const n=require('node:net');const s=n.createServer();s.listen(0,'127.0.0.1',()=>{const a=s.address();process.stdout.write(String(a.port));s.close();});"],
    { env: { ...process.env, ELECTRON_RUN_AS_NODE: "1" }, encoding: "utf8" },
  );
  const port = Number(String(result.stdout || "").trim());
  if (result.status !== 0 || !Number.isSafeInteger(port) || port < 1 || port > 65_535) {
    throw new Error("runtime_endpoint_allocate_failed");
  }
  return `http://127.0.0.1:${port}`;
}
function lifecycleEnvironment(env = process.env) {
  const explicit = normalizeLifecycleEndpoint(env.ECHODESK_BASE_URL);
  const record = explicit ? null : liveRuntimeRecordEndpoint(env);
  const endpoint = explicit || record || allocateLifecycleEndpoint();
  return { ...env, ECHODESK_BASE_URL: endpoint };
}
const RUNTIME_ENV = lifecycleEnvironment(process.env);
const BACKEND_ENDPOINT = resolveBackendEndpoint(backendConfig, RUNTIME_ENV, {
...
// Release defaults to the bundled local backend. Remote/public mode remains an
// explicit principal selection and is never a fallback for local data.
...
// ---------- 运行时状态 ----------
const backendLifecycle = createBackendLifecycleController();
let mainWindow = null;
let tray = null;
...
let publicIdentityVault = null;
let publicSessionEnsurePromise = null;
let currentPublicSession = null;
const bootStageState = {
  appReady: false,
  vaultLoad: false,
  sessionBootstrap: false,
  windowCreated: false,
  backendSpawn: false,
  cdpBound: process.argv.some((arg) => /^--remote-debugging-port=\d+$/.test(arg)),
  errorClass: null,
};

function emitBootStage(stage, errorClass = null) {
  if (stage in bootStageState) bootStageState[stage] = true;
  if (errorClass) bootStageState.errorClass = String(errorClass);
  log(`[boot-stage] ${JSON.stringify(bootStageState)}`);
}
...
function clearPublicCredential() {
  ...
  publicIdentityVault = null;
  publicSessionEnsurePromise = null;
  currentPublicSession = null;
}

function publicIdentityCredentialVault() {
  if (publicIdentityVault === null) {
    publicIdentityVault = createCredentialVault({
      safeStorage,
      target: path.join(app.getPath("userData"), "public-device-identity.bin"),
      backendBase: BACKEND_HOST,
      officialBackendBase: PUBLIC_BACKEND_HOST,
      enabled: PUBLIC_DEMO_MODE,
      platform: process.platform,
      logger: (message) => log(`[identity] ${message}`),
    });
    emitBootStage("vaultLoad");
  }
  return publicIdentityVault;
}
...
function publicIdentitySessionManager() {
  if (publicIdentityManager === null) {
    publicIdentityManager = createPublicIdentitySessionManager({
      vault: publicIdentityCredentialVault(),
...
async function enrollPublicSessionExplicitly() {
...
async function ensurePublicSessionInMain({ forceBootstrap = false } = {}) {
  ...
  if (forceBootstrap) {
    ...
    // A renderer reload must revalidate the same origin-bound identity. It
    // must never silently enroll a replacement owner, because durable capture
    // partitions are fenced to the enrolled principal/device.
    currentPublicSession = null;
  }
  ...
  const pending = (async () => {
    ...
    emitBootStage("sessionBootstrap");
    if (session) {
...
async function rotatePublicCredential() {
  return publicIdentitySessionManager().rotate(currentPublicSession?.token || "");
}
...
function parseMinutesJson(raw) {
  ...
  try {
    ...
    if (typeof parsed.created_at === "string") {
      parsed.created_at = normalizeLegacyTimestamp(parsed.created_at);
    }
    if (Array.isArray(parsed.todos)) {
      for (const todo of parsed.todos) {
        if (todo && typeof todo.done_at === "string") {
          todo.done_at = normalizeLegacyTimestamp(todo.done_at);
        }
      }
    }
    return parsed;
...
function normalizeLegacyTimestamp(value) {
  ...
  if (/^\d{4}-\d\d-\d\d[ T]\d\d:\d\d:\d\d(?:\.\d+)?$/.test(trimmed)) {
...
function legacyArtifactExtension(artifactType, mimeType) {
  const byType = {
    html: "html",
    markdown: "md",
    pdf: "pdf",
  ...
  if (byType[String(artifactType || "").toLowerCase()]) {
    return byType[String(artifactType).toLowerCase()];
  }
...
function artifactRecordFromRow(row, links = []) {
  return {
    artifact_id: String(row.artifact_id),
    artifact_type: String(row.artifact_type || "file"),
    title: String(row.title || ""),
    file_path: row.file_path ? String(row.file_path) : null,
    mime_type: String(row.mime_type || "application/octet-stream"),
    size_bytes: Number(row.size_bytes) || 0,
    generation_latency_ms: Number(row.generation_latency_ms) || 0,
    model: String(row.model || ""),
    metadata: parseJsonObject(row.metadata_json),
    run_id: row.run_id || null,
    created_at: normalizeLegacyTimestamp(row.created_at) || null,
    updated_at: normalizeLegacyTimestamp(row.updated_at) || null,
    links,
    file_extension: legacyArtifactExtension(row.artifact_type, row.mime_type),
  };
}

function loadLegacyEchodeskHistory() {
  const dbPath = findLegacyEchodeskDb();
  ...
  const rawSegments = sqliteCliJson(
    dbPath,
  ...
  for (const seg of rawSegments) {
    ...
    list.push({
      ...
      captured_at: seg.captured_at || null,
    });
    segmentsByMeeting.set(seg.meeting_id, list);
  }

  const rawArtifacts = tables.includes("artifacts")
    ? sqliteCliJson(
        dbPath,
        `SELECT artifact_id, artifact_type, title, file_path, mime_type,
    ...
    : [];
  const rawArtifactLinks = tables.includes("artifact_links")
    ? sqliteCliJson(
        dbPath,
        `SELECT link_id, artifact_id, source, meeting_id, todo_id, run_id, created_at
           FROM artifact_links
          ORDER BY created_at ASC, link_id ASC`,
      )
    : [];
  const linksByArtifact = new Map();
  const artifactsByMeeting = new Map();
  for (const link of rawArtifactLinks) {
    if (!link.artifact_id) continue;
    const normalized = {
      link_id: String(link.link_id || ""),
      artifact_id: String(link.artifact_id),
      source: String(link.source || "legacy-history"),
      meeting_id: link.meeting_id || null,
      todo_id: link.todo_id || null,
      run_id: link.run_id || null,
      created_at: normalizeLegacyTimestamp(link.created_at) || null,
    };
    const links = linksByArtifact.get(normalized.artifact_id) || [];
    links.push(normalized);
    linksByArtifact.set(normalized.artifact_id, links);
    if (normalized.meeting_id) {
      const meetingArtifacts = artifactsByMeeting.get(normalized.meeting_id) || [];
      meetingArtifacts.push(normalized.artifact_id);
      artifactsByMeeting.set(normalized.meeting_id, meetingArtifacts);
    }
  }
  const artifacts = rawArtifacts.map((row) =>
    artifactRecordFromRow(row, linksByArtifact.get(String(row.artifact_id)) || []),
  );
  ...
  const meetings = rawMeetings
    ...
    .map((row) => {
      ...
      const linkedArtifacts = (artifactsByMeeting.get(String(row.id)) || [])
      ...
      return {
        ...
        started_at: normalizeLegacyTimestamp(row.started_at) || undefined,
        ended_at:
          normalizeLegacyTimestamp(row.ended_at || row.finalized_at) || undefined,
        segments,
        speakers,
  ...
  const ambientSegments = tables.includes("ambient_segments")
    ? sqliteCliJson(
        ...
        .map((row) => ({
          ...
          captured_at:
            normalizeLegacyTimestamp(row.captured_at) || new Date().toISOString(),
  ...
  return {
    ...
    summaryCount: rawMeetings.filter((row) => Boolean(parseMinutesJson(row.minutes_json))).length,
    artifactCount: artifacts.length,
  };
}

function loadLegacyEchodeskHistoryPage(phase, offset, limit = 20) {
  const dbPath = findLegacyEchodeskDb();
  if (!dbPath) return null;
  const tables = sqliteCliJson(
    dbPath,
  ...
  const phases = new Set(["meetings", "segments", "summaries", "artifacts"]);
  const currentPhase = String(phase || "");
  if (!phases.has(currentPhase)) throw new Error("invalid legacy history sync phase");
  const start = Number(offset);
  const pageSize = Number(limit);
  if (!Number.isSafeInteger(start) || start < 0) throw new Error("invalid legacy history sync offset");
  if (!Number.isSafeInteger(pageSize) || pageSize < 1 || pageSize > 20) {
    throw new Error("invalid legacy history sync page size");
  }

  const page = (sql, mapper) => {
    const rows = sqliteCliJson(
      dbPath,
    ...
    return {
      ...
      ...page(
        `SELECT id, title, state, started_at, ended_at, finalized_at,
          ...
          ORDER BY started_at DESC`,
        (row) => ({
          operation_id: `legacy:meeting:${String(row.id)}`,
          entity_type: "meeting",
          entity_id: String(row.id),
          updated_at:
            normalizeLegacyTimestamp(
              row.finalized_at || row.ended_at || row.started_at,
            ) || new Date().toISOString(),
          payload: {
            meeting_id: String(row.id),
            title: row.title || row.id,
            display_title: row.display_title || null,
            state: row.state === "in_meeting" ? "in_meeting" : row.state || "ended",
            started_at: normalizeLegacyTimestamp(row.started_at) || null,
            ended_at:
              normalizeLegacyTimestamp(row.ended_at || row.finalized_at) || null,
            finalized_at: normalizeLegacyTimestamp(row.finalized_at) || null,
            minutes_status: row.minutes_status || null,
            minutes_error: row.minutes_error || null,
          },
        }),
      ),
  ...
  if (currentPhase === "segments") {
    return {
      phase: currentPhase,
      offset: start,
      ...page(
        `SELECT id, meeting_id, text, start_ms, end_ms, speaker_id,
                speaker_label, captured_at
           FROM meeting_segments
          ORDER BY id ASC`,
        (row) => ({
          operation_id: `legacy:segment:${String(row.id)}`,
          entity_type: "transcript_segment",
          entity_id: `legacy-segment-${String(row.id)}`,
          updated_at: normalizeLegacyTimestamp(row.captured_at) || new Date().toISOString(),
          payload: {
            ...
            speaker_id: row.speaker_id || null,
            speaker_label: row.speaker_label || null,
            captured_at: normalizeLegacyTimestamp(row.captured_at) || null,
          },
        }),
      ),
    };
  }
  if (currentPhase === "summaries") {
    return {
      phase: currentPhase,
      offset: start,
      ...page(
        `SELECT id, minutes_json, finalized_at, ended_at, started_at
           FROM meetings
          WHERE minutes_json IS NOT NULL AND length(minutes_json) > 0
          ORDER BY id ASC`,
        (row) => ({
          operation_id: `legacy:summary:${String(row.id)}`,
          entity_type: "meeting_summary",
          entity_id: String(row.id),
          updated_at: row.finalized_at || row.ended_at || row.started_at || new Date().toISOString(),
          payload: {
            ...(parseMinutesJson(row.minutes_json) || {}),
            meeting_id: String(row.id),
          },
        }),
      ),
  ...
  if (!tables.includes("artifacts")) {
    return { phase: currentPhase, offset: start, items: [], nextOffset: start, done: true };
  }
  const artifactPage = sqliteCliJson(
    dbPath,
    `SELECT artifact_id, artifact_type, title, file_path, mime_type,
            size_bytes, generation_latency_ms, model, metadata_json,
            run_id, created_at, updated_at
  ...
  const ids = artifactPage.map((row) => row.artifact_id).filter(Boolean);
  const linkRows = ids.length > 0
    ? sqliteCliJson(
        dbPath,
        `SELECT link_id, artifact_id, source, meeting_id, todo_id, run_id, created_at
           FROM artifact_links
          WHERE artifact_id IN (${ids.map(sqlQuote).join(",")})
          ORDER BY created_at ASC, link_id ASC`,
      )
    : [];
  const pageLinks = new Map();
  for (const link of linkRows) {
    const list = pageLinks.get(String(link.artifact_id)) || [];
    list.push({
      link_id: String(link.link_id || ""),
      artifact_id: String(link.artifact_id),
      source: String(link.source || "legacy-history"),
    ...
    pageLinks.set(String(link.artifact_id), list);
  }
  const items = artifactPage.map((row) => {
    const artifact = artifactRecordFromRow(
      row,
      pageLinks.get(String(row.artifact_id)) || [],
    );
    ...
    return {
      operation_id: `legacy:artifact:${String(row.artifact_id)}`,
      entity_type: "artifact",
      entity_id: String(row.artifact_id),
      updated_at:
        normalizeLegacyTimestamp(row.updated_at || row.created_at) ||
        new Date().toISOString(),
      payload: {
        ...
        generation_latency_ms: artifact.generation_latency_ms,
        ...
        content_sha256: contentBase64
          ? require("node:crypto").createHash("sha256").update(Buffer.from(contentBase64, "base64")).digest("hex")
          : null,
      },
    };
  });
  return {
    ...
    offset: start,
    items,
    nextOffset: start + artifactPage.length,
    done: artifactPage.length < pageSize,
  };
}
...
// 不加 -sTCP:LISTEN 会把 client side 也返回，导致误判端口被占。
...
async function performBackendHealthcheck() {
  if (PUBLIC_DEMO_MODE) {
    try {
      ...
      emitBootStage("sessionBootstrap", code || safeFailureCode(error));
...
function startHealthWatcher(lifecycleLease) {
  if (healthTimer) return;
  const scheduleNext = () => {
    if (shuttingDown || !backendLifecycle.isCurrent(lifecycleLease)) return;
    healthTimer = setTimeout(runHealthTick, HEALTH_INTERVAL_MS);
  };
  const runHealthTick = async () => {
    const ownedTimer = healthTimer;
    if (
      shuttingDown ||
      !ownedTimer ||
      !backendLifecycle.isCurrent(lifecycleLease)
    ) return;
    const ok = await healthcheckOnce();
    // A health request can settle after stop/restart. Its result belongs only
    // to the child and generation captured when this watcher was created.
    if (
      shuttingDown ||
      healthTimer !== ownedTimer ||
      !backendLifecycle.isCurrent(lifecycleLease)
    ) return;
    const outcome = await backendLifecycle.settleHealth(
      lifecycleLease,
      ok,
    );
    if (
    ...
    if (outcome.state === "starting") {
      // 启动期：uvicorn 还没绑 socket 是正常的，不计入失败计数；但要给一个硬超时
      if (backendLifecycle.elapsedMs(lifecycleLease) > STARTUP_TIMEOUT_MS) {
        log("[backend] startup timeout (never became ready)");
        void handleBackendDeath("startup timeout", lifecycleLease);
        return;
      }
      scheduleNext();
      return;
    ...
    if (outcome.state === "failed") {
      log(`[backend] healthz fail ${outcome.failures}/${HEALTH_FAIL_THRESHOLD}`);
      if (outcome.failures >= HEALTH_FAIL_THRESHOLD) {
        void handleBackendDeath(
          `healthz failed ${HEALTH_FAIL_THRESHOLD}x`,
          lifecycleLease,
        );
        return;
      }
...
async function killBackendProc(expectedLease = null) {
  const stopped = backendLifecycle.invalidate(expectedLease);
  if (!stopped?.child || stopped.child.exitCode !== null) return stopped;
  try {
    await stopBackendProcess(stopped.child, { graceMs: SIGKILL_GRACE_MS });
  } catch (error) {
...
function stopBackendProcForRestart() {
  ...
  const stopped = backendLifecycle.invalidate();
  if (!stopped?.child || stopped.child.exitCode !== null) return Promise.resolve();
  return stopBackendProcess(stopped.child, { graceMs: SIGKILL_GRACE_MS });
}

async function handleBackendDeath(reason, expectedLease = null) {
  if (
    shuttingDown ||
    (expectedLease && !backendLifecycle.isCurrent(expectedLease))
  ) return;
  stopHealthWatcher();
  backendHealthcheckPromise = null;
  const stopped = await killBackendProc(expectedLease);
  if (!stopped) return;
  const lifecycleGeneration = stopped.invalidatedByGeneration;
  if (
    shuttingDown ||
    !backendLifecycle.isGenerationCurrent(lifecycleGeneration)
  ) return;
...
function spawnBackendAndWatch() {
  if (shuttingDown) return;
  const runningChild = backendLifecycle.currentChild();
  if (runningChild && runningChild.exitCode === null && !runningChild.killed) {
  ...
  backendHealthcheckPromise = null;
  const spawnLease = backendLifecycle.beginSpawn();
  let spawnedChild;
  try {
    const inheritedBackendEnv = (bundledBackend && process.platform === "win32")
      ? sanitizedWindowsPackagedBackendEnv(process.env)
      : process.env;
    // 源码开发使用 monorepo 内可编辑的 yoli_llm；打包 backend 必须保持其
    // frozen runtime，不继承此源码路径。保留调用方已有 PYTHONPATH，避免
    // 覆盖本地开发所需的其它显式模块目录。
    const sourceBackendPythonPath = bundledBackend
      ? null
      : [
          path.join(projectRoot(), "..", "_platforms", "llm", "src"),
          inheritedBackendEnv.PYTHONPATH,
    ...
    spawnedChild = spawn(
      ...
      args,
      {
        ...
        env: {
          ...inheritedBackendEnv,
          ...(sourceBackendPythonPath ? { PYTHONPATH: sourceBackendPythonPath } : {}),
          // The supervisor-selected endpoint is the only backend endpoint
          // source.  The child publishes this same injected value after it is
          // ready for independently launched local harnesses.
          ECHODESK_BASE_URL: LOCAL_BACKEND_HOST,
          ...
          https_proxy: "",
          all_proxy: "",
          // httpx 0.28 会把 NO_PROXY 里的裸 IPv6 `::1` 解析成非法端口
          // `:1`，使 SpeechBrain/Hugging Face 在创建 client 时直接失败。
          // backend 只需绕过本机 HTTP 服务；使用无歧义的 loopback 列表。
          NO_PROXY: "localhost,127.0.0.1",
          no_proxy: "localhost,127.0.0.1",
        },
        // Backend stdout/stderr are drained below; Windows packaged GUI apps do
        // not have a stable console, so bytes go to files instead of process
        // stdout/process.stderr there.
    ...
    void handleBackendDeath("spawn failed", spawnLease);
    return;
  }

  const lifecycleLease = backendLifecycle.attachChild(spawnLease, spawnedChild);
  if (!lifecycleLease) {
    void stopBackendProcess(spawnedChild, { graceMs: SIGKILL_GRACE_MS });
    return;
  }
  ...
  spawnedChild.on("exit", (code, signal) => {
    const wasOurs = backendLifecycle.isCurrent(lifecycleLease);
    log(`[backend] child exited code=${code} signal=${signal} ours=${wasOurs}`);
    if (wasOurs && !shuttingDown) {
      // Only the currently owned child may trigger replacement. An old child's
      // late exit is an inert historical callback.
      void handleBackendDeath(`child exited code=${code}`, lifecycleLease);
    }
  });

  startHealthWatcher(lifecycleLease);
}
...
async function createWindow({ showOnReady = true } = {}) {
  ...
  // `ready-to-show` is best-effort: a packaged renderer can finish its
  // trusted navigation without emitting that event (for example while the
  // first compositor frame is deferred).  Keep normal hidden/background
  // launches hidden, but never leave a user-facing launch or activation with
  // a BrowserWindow that exists only in the main-process state.
...
ipcMain.handle("credential:enroll-session", async (event) => {
  assertTrustedIpcOrigin(event);
  return enrollPublicSessionExplicitly();
});
...
ipcMain.handle("echo:load-local-legacy-history-page", async (event, phase, offset, limit) => {
  assertTrustedIpcOrigin(event);
  if (process.platform !== "darwin") return null;
  try {
    return loadLegacyEchodeskHistoryPage(phase, offset, limit);
  } catch (e) {
    log(`[legacy-history] sync page failed: ${safeFailureCode(e)}`);
    throw new Error("legacy history sync page unavailable");
  }
});
  ...
  suggestedFilename,
    ...
    try {
      return await downloadRendererBlob({
        ...
        logger: (message) => log(`[artifact-download] ${message}`),
      });
    } catch (cause) {
      log(
        `[artifact-download] failed code=${safeFailureCode(cause)}`,
      );
...
if (singleInstanceLock) app.whenReady()
  .then(async () => {
    ...
    if (!IS_DEV) {
      installAppProtocol(
        ...
        {
          backendBase: BACKEND_HOST,
          pairedHubBase: BACKEND_ENDPOINT.pairedHubSyncGatewayEndpoint,
        },
      );
    }
    installMediaPermissionHandlers(session.defaultSession, {
      ...
      reportDecision: (phase, decision) => {
        log(`[media-permission] ${phase} ${JSON.stringify(decision)}`);
      },
    });
  ...
  .catch((error) => {
    emitBootStage("errorClass", safeFailureCode(error));
...
app.on("window-all-closed", () => {
  // The supervised backend, public session and sync worker are app-scoped. Keep
  // them alive while the user has merely hidden every window.
});
