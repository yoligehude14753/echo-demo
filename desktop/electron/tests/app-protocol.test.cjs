...
test("production CSP grants only the bound backend and has no retired bootstrap asset", () => {
  const csp = productionContentSecurityPolicy("http://127.0.0.1:19345");
  assert.match(csp, /connect-src 'self' http:\/\/127\.0\.0\.1:19345 ws:\/\/127\.0\.0\.1:19345/);
  ...
  assert.equal(
    existsSync(path.resolve(__dirname, "../../public/boot-fallback.js")),
    false,
  );
...
test("production CSP grants the packaged paired Hub only for HTTP and WebSocket connections", () => {
  ...
  assert.match(
    csp,
    /connect-src 'self' http:\/\/127\.0\.0\.1:19345 ws:\/\/127\.0\.0\.1:19345 https:\/\/echo\.yoliyoli\.uk wss:\/\/echo\.yoliyoli\.uk/,
  );
  assert.doesNotMatch(csp.match(/img-src[^;]*/)?.[0] ?? "", /echo\.yoliyoli\.uk/);
  assert.doesNotMatch(csp.match(/media-src[^;]*/)?.[0] ?? "", /echo\.yoliyoli\.uk/);
  assert.doesNotMatch(csp, /https:\s|wss:\s|\*/);
});
...
test("main process registers before ready and never falls back to file loading", () => {
  ...
  assert.match(main, /pairedHubBase: BACKEND_ENDPOINT\.pairedHubSyncGatewayEndpoint/);
