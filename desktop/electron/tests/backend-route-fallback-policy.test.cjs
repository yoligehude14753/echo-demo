...
test("desktop package has exactly one canonical Electron main entry", () => {
  const packageJson = require(path.resolve(__dirname, "../../package.json"));
  const retiredTypeScriptEntry = path.resolve(__dirname, "../main.ts");
  const retiredCompiledEntryDirectory = path.resolve(__dirname, "../../dist-electron");

  assert.equal(packageJson.main, "electron/main.cjs");
  assert.equal(existsSync(retiredTypeScriptEntry), false);
  assert.equal(existsSync(retiredCompiledEntryDirectory), false);
});
...
test("release local routing owns the loopback endpoint and bundled backend", () => {
  const endpoint = require("../backend-endpoint.cjs");
  const config = require(path.resolve(__dirname, "../../backend.config.json"));
  const runtime = endpoint.resolveBackendEndpoint(config, { ECHODESK_BASE_URL: "http://127.0.0.1:19345" });
  assert.equal(runtime.mode, "local");
  assert.equal(config.roles.publicService.baseUrl, CANONICAL_PUBLIC_ORIGIN);
  assert.equal(runtime.backendBase, "http://127.0.0.1:19345");
  assert.equal(runtime.publicServiceEndpoint, CANONICAL_PUBLIC_ORIGIN);
  assert.equal(runtime.spawnBackend, true);
  assert.equal(runtime.localBase, "http://127.0.0.1:19345");
  assert.equal(runtime.localDevDiagnosticEndpoint, "http://127.0.0.1:19345");
  assert.equal(runtime.bindHost, "127.0.0.1");
});

test("release desktop, renderer, and LLM routing roots emit only the canonical public origin", () => {
  const roots = [
