...
test("frozen backend rejects every retired application module", () => {
  assert.deepEqual(
    FORBIDDEN_LEGACY_APP_MODULES.filter((entry) =>
      [
...
test("frozen backend manifest preserves CPU diarization and rejects unused runtimes", () => {
  ...
  try {
    ...
    writeFileSync(
      manifest,
      `[${required}, ('${FORBIDDEN_LEGACY_APP_MODULES[0]}', '/tmp/legacy.py', 'PYMODULE')]\n`,
    );
    assert.throws(
      () => verifyFrozenAnalysis(manifest),
      (error) => error instanceof Error
        && error.message.includes("legacy application path")
        && error.message.includes(FORBIDDEN_LEGACY_APP_MODULES[0]),
    );
    writeFileSync(
      manifest,
