...
const audioCapture = readFileSync(
  path.resolve(__dirname, "../../src/capture/audioCapture.ts"),
  "utf8",
);
...
test("renderer preflights macOS microphone access through a trusted IPC channel", () => {
  const body = registrationBody("mic:request");
  assert.match(body, /assertTrustedIpcOrigin\(event\)/);
  assert.match(body, /systemPreferences\.getMediaAccessStatus\("microphone"\)/);
  assert.match(body, /status === "granted"/);
  assert.match(body, /systemPreferences\.askForMediaAccess\("microphone"\)/);
  assert.match(preload, /requestMic:\s*\(\) => ipcRenderer\.invoke\("mic:request"\)/);
  assert.match(audioCapture, /navigator\.mediaDevices\s*\.getUserMedia/);
  assert.match(audioCapture, /getUserMediaWithTimeout/);
  assert.match(audioCapture, /settleElectronMicIpc\(window\.echo\?\.getMicStatus\)/);
  assert.match(audioCapture, /settleElectronMicIpc\(window\.echo\?\.requestMic\)/);
  assert.match(audioCapture, /await requestElectronMicAccess\(\);[\s\S]*?getUserMediaWithTimeout/);
});

test("backend proxy environment keeps loopback bypass parseable by httpx", () => {
  assert.match(main, /HTTP_PROXY:\s*""/);
  assert.match(main, /HTTPS_PROXY:\s*""/);
  assert.match(main, /ALL_PROXY:\s*""/);
  assert.match(main, /NO_PROXY:\s*"localhost,127\.0\.0\.1"/);
  assert.match(main, /no_proxy:\s*"localhost,127\.0\.0\.1"/);
  assert.doesNotMatch(main, /NO_PROXY:\s*"[^"]*::1/);
});
