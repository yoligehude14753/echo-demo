...
test("current promotion remains a strict updater-compatible metadata step", () => {
  const result = assertVersionContract();
  assert.equal(result.previousVersion, "0.3.25");
  assert.equal(result.targetVersion, TARGET_VERSION);
  assert.equal(result.androidVersionCode, TARGET_VERSION_CODE);
  assert.deepEqual(result.assets, {
    darwin: "EchoDesk-0.3.26-arm64-mac.zip",
    win32: "EchoDesk.Setup.0.3.26.exe",
    android: "EchoDesk-0.3.26-android.apk",
  });
  assert.equal(result.releaseChannel, "stable");
  assert.equal(result.releaseNotes, "EchoDesk 0.3.26");
...
test("release readback requires exact tag SHA and exactly three canonical update assets", () => {
  ...
  assert.match(evidence.releaseNotes, new RegExp(TARGET_VERSION.replaceAll(".", "\\.")));
