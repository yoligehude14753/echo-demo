...
test("top bar names the event stream without implying HTTP capture stopped", () => {
  assert.match(appSource, /实时事件通道正在连接；音频上传使用独立 HTTP 通道/);
  assert.match(appSource, /connected \? "实时事件" : "事件重连中"/);
  assert.doesNotMatch(appSource, /connected \? "已连接" : "断线"/);
});
