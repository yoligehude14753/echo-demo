...
test("durable public identity reuses the same principal after an Electron restart", async () => {
  const { root, vault } = fixture();
  try {
    const requests = [];
    const nextManager = (token) => createPublicIdentitySessionManager({
      vault,
    ...
    const first = nextManager("first-token");
    await first.ensure();
    const restarted = nextManager("restarted-token");
    ...
    assert.equal(requests[1].body.device_credential, requests[0].body.device_secret);
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
  }
});
...
test("missing durable identity fails closed when enrollment is not explicit", async () => {
  const { root, vault } = fixture();
  try {
    let requests = 0;
    const manager = createPublicIdentitySessionManager({
    ...
    await assert.rejects(manager.ensure(), (error) => {
      assert.equal(error.code, "IDENTITY_STORE_MISSING");
      return true;
    });
    assert.equal(requests, 0);
