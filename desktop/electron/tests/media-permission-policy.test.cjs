...
function installedPolicy() {
  ...
  installMediaPermissionHandlers(
    ...
    },
    {
      ...
      reportDecision: (phase, decision) => decisions.push({ phase, decision }),
    },
  );
  return { requestHandler, checkHandler, decisions };
}
...
test("permission check handler binds requesting origin and frame details", () => {
  const { checkHandler, decisions } = installedPolicy();
  ...
  assert.equal(decisions[0].phase, "check");
  assert.deepEqual(decisions[0].decision, {
    permission: "media",
    allowed: true,
    isMainFrame: true,
    mediaType: "audio",
    hasRequestingUrl: true,
  });
});
