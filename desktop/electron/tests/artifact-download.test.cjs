...
test("bounded renderer blob download writes a real file and returns no path", async (t) => {
  ...
  const result = await request(downloads, harness, {
    logger: (line) => audit.push(JSON.parse(line)),
  });
  ...
  assert.deepEqual(audit.map((item) => item.event), [
...
test("unknown-size blob downloads are accepted and still bounded", async (t) => {
  for (const totalBytes of [-1, Number.NaN]) {
    const downloads = temporaryDownloads(t);
    const marker = Buffer.from("UNKNOWN_SIZE_BLOB", "utf8");
    const harness = downloadHarness({ totalBytes, bytes: marker, receivedBytes: marker.length });
    const result = await request(downloads, harness);
    assert.equal(result.ok, true);
    assert.deepEqual(fs.readFileSync(path.join(downloads, result.filename)), marker);
  }
});

test("oversized downloads fail closed without partial files", async (t) => {
  const downloads = temporaryDownloads(t);
  const harness = downloadHarness({
    totalBytes: MAX_ARTIFACT_DOWNLOAD_BYTES + 1,
