...
const { requestBoundedHttpsJson } = require("./bounded-https-json.cjs");
...
async function backendBoundJsonFetch({
  ...
  try {
    // Test-only compatibility injection. Runtime callers leave this unset and
    // therefore never enter Electron/undici global fetch.
    if (typeof fetchImpl === "function") {
      const timer = setTimer(() => controller.abort(new DOMException("identity request timed out", "TimeoutError")), timeoutMs);
      try {
        const response = await fetchImpl(target, { method, headers, body, signal: controller.signal, redirect: "error" });
        ...
        return await boundedIdentityResponse(response, controller.signal, maxResponseBytes);
      } finally { clearTimer(timer); }
    }
    const response = requestImpl
      ? await requestBoundedHttpsJson(target, { method, headers, body, maxBytes: maxResponseBytes, timeoutMs, rejectHttpErrors: false, signal: controller.signal, requestImpl, setTimer, clearTimer })
      : await requestBoundedHttpsJson(target, { method, headers, body, maxBytes: maxResponseBytes, timeoutMs, rejectHttpErrors: false, signal: controller.signal });
    if (response.status >= 300 && response.status < 400) {
      await cancelResponseBody(response);
      const error = new Error("backend-bound redirects are forbidden");
      error.code = "IDENTITY_BACKEND_REDIRECT_FORBIDDEN";
    ...
    if (response.body !== undefined) return { ...response, async json() { return response.body; } };
    return await boundedIdentityResponse(response, controller.signal, maxResponseBytes);
  } finally {
...
function requireStoredState(vault) {
  try {
    ...
    if (error instanceof PublicIdentitySessionError) throw error;
    throw new PublicIdentitySessionError(
      "encrypted credential storage could not be read",
      "IDENTITY_STORE_READ_FAILED",
      { cause: error },
    );
  }
}
...
function createPublicIdentitySessionManager({
  ...
  async function ensure() {
    if (allowEnrollment !== true && !vault.exists()) {
      throw new PublicIdentitySessionError(
        "stored device identity is required; explicit enrollment is disabled",
