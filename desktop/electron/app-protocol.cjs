...
function originConnectionSources(rawBase) {
  if (!rawBase) return [];
  try {
    const origin = new URL(String(rawBase));
    if (
    ...
    // Invalid endpoint configuration is rejected elsewhere and grants no CSP source.
    return [];
  }
}
...
function productionContentSecurityPolicy(
  rawBackendBase = null,
  rawPairedHubBase = null,
) {
  const backendSources = originConnectionSources(rawBackendBase);
  const connectSources = [
    ...new Set([
      ...backendSources,
      ...originConnectionSources(rawPairedHubBase),
    ]),
  ];
...
function responseHeaders(
  ...
  if (extension === ".html") {
    headers["Content-Security-Policy"] =
      productionContentSecurityPolicy(backendBase, pairedHubBase);
...
function installAppProtocol(
  protocol,
  distRoot,
  { backendBase = null, pairedHubBase = null } = {},
) {
  protocol.handle(
    APP_SCHEME,
    createAppProtocolHandler({ distRoot, backendBase, pairedHubBase }),
  );
}
