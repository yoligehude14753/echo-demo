...
test("S07b · 任何输入 → 不再显示 intent-status 后台信息（置信度/关键字命中等）", async ({
  page,
}) => {
  // 用户 2026-05-28：「置信度，还有关键字命中，这些后台信息不要显示了」
  // 回归断言：意图路由仍然走（routeIntent 仍被调用），但 UI 不渲染 intent-status 行。
  await page.route(/\/(api\/)?intent\/route$/, async (route) => {
    await route.fulfill({
      status: 200,
      contentType: "application/json",
  ...
  await expect(page.getByTestId("intent-status")).toHaveCount(0);
  ...
  await expect(page.locator("body")).not.toContainText("关键字命中");
});
