import { expect, test } from "@playwright/test";
import { readFileSync } from "node:fs";
import { dirname, resolve } from "node:path";
import { fileURLToPath } from "node:url";
import { installEchoMock } from "./_mock";

const currentDir = dirname(fileURLToPath(import.meta.url));
const packageVersion = JSON.parse(
  readFileSync(resolve(currentDir, "../../package.json"), "utf8"),
).version as string;

test("验收点击流：知识库、设置、模型服务状态、移动连接和输入框均可操作", async ({ page }) => {
  await page.setViewportSize({ width: 1280, height: 820 });
  const hiddenDefaultModelUrl = ["https://yu", "nwu.ai/v1"].join("");

  const docs = [
    {
      doc_id: "doc-acceptance-1",
      title: "EchoDesk 验收清单.md",
      kind: "md",
      source: "workspace",
      source_path: "/Users/test/work/EchoDesk 验收清单.md",
      n_chunks: 5,
    },
  ];
  let savedMobileBase = "";

  await page.route(/\/(api\/)?healthz\/full$/, async (route) => {
    await route.fulfill({
      status: 200,
      contentType: "application/json",
      body: JSON.stringify({
        backend: { ok: true, version: "0.2.6-mock", port: 8769, uptime_s: 12.3 },
        db: { ok: true },
        remote: {
          speech_recognition: { ok: true, latency_ms: 18, checked_at: Date.now() / 1000 },
          speech_synthesis: { ok: true, latency_ms: 22, checked_at: Date.now() / 1000 },
          fast_model: { ok: true, latency_ms: 35, checked_at: Date.now() / 1000 },
          main_model: { ok: true, latency_ms: 60, checked_at: Date.now() / 1000 },
          web_search: { ok: null, reason: "no_api_key", checked_at: Date.now() / 1000 },
        },
        mic: { ok: "unknown" },
      }),
    });
  });

  await page.route(/\/(api\/)?admin\/settings\/remote$/, async (route) => {
    const req = route.request();
    if (req.method() === "PATCH") {
      await route.fulfill({
        status: 200,
        contentType: "application/json",
        body: JSON.stringify({ status: "ok", restart_required: true, updated: 1 }),
      });
      return;
    }
    await route.fulfill({
      status: 200,
      contentType: "application/json",
      body: JSON.stringify({
        config_path: "/Users/test/.echodesk/config.json",
        fields: [
          { key: "llm_main_base_url", value: hiddenDefaultModelUrl, sensitive: false, source: "default" },
          { key: "yunwu_open_key", value: "", sensitive: true, source: "default" },
          {
            key: "llm_fast_base_url",
            value: hiddenDefaultModelUrl,
            sensitive: false,
            source: "default",
          },
          {
            key: "stt_firered_url",
            value: "http://100.76.3.59:8090",
            sensitive: false,
            source: "default",
          },
          {
            key: "tts_qwen3_url",
            value: "http://100.76.3.59:8094",
            sensitive: false,
            source: "default",
          },
          { key: "tts_qwen3_voice", value: "aiden", sensitive: false, source: "default" },
          { key: "tavily_api_key", value: "", sensitive: true, source: "default" },
        ],
      }),
    });
  });

  await page.route(/\/(api\/)?admin\/data-dir$/, async (route) => {
    await route.fulfill({
      status: 200,
      contentType: "application/json",
      body: JSON.stringify({
        path: "/Users/test/.echodesk",
        exists: true,
        size_bytes: 4096,
        breakdown: {
          db: 1024,
          storage: 0,
          rag_index: 2048,
          logs: 1024,
          skill_build: 0,
        },
      }),
    });
  });

  await page.route(/\/(api\/)?workspace\/status$/, async (route) => {
    await route.fulfill({
      status: 200,
      contentType: "application/json",
      body: JSON.stringify({
        configured_dirs: ["/Users/test/work"],
        authorized_dirs: ["/Users/test/work"],
        n_indexed: 1,
        max_file_mb: 100,
        scan_on_startup: true,
      }),
    });
  });

  await page.route(/\/(api\/)?rag\/docs(\/[^?]+)?$/, async (route) => {
    await route.fulfill({
      status: 200,
      contentType: "application/json",
      body: JSON.stringify({ total: docs.length, by_source: { workspace: docs }, docs }),
    });
  });

  await installEchoMock(page, {
    skipPaths: [
      "/healthz/full",
      "/admin/settings/remote",
      "/admin/data-dir",
      "/workspace/status",
      "/rag/docs",
    ],
  });
  await page.goto("/", { waitUntil: "domcontentloaded" });

  await expect(page.getByTestId("open-about")).toContainText(`v${packageVersion}`);
  await page.getByTestId("pill-model-service").click();
  await expect(page.getByText("语音与模型服务")).toBeVisible();
  await page.keyboard.press("Escape");

  await expect(page.getByTestId("workspace-config-btn")).toContainText("配置工作区");
  await page.getByTestId("workspace-dirs-tag").click();
  await expect(page.getByText("知识库 / 工作区文件")).toBeVisible();
  await expect(
    page.getByTestId("knowledge-doc-row").getByText("EchoDesk 验收清单.md", { exact: true }),
  ).toBeVisible();

  await page.getByTestId("workspace-open-settings").click();
  await expect(page.getByTestId("workspace-settings-section")).toBeVisible();
  await expect(page.getByTestId("workspace-add-dir")).toBeFocused();
  await expect(page.getByTestId("remote-settings-form")).toBeVisible();
  await expect(page.locator("body")).not.toContainText("激活码");

  await expect(page.getByRole("textbox", { name: "STT URL" })).toHaveValue("");
  await expect(page.getByRole("textbox", { name: "TTS URL" })).toHaveValue("");
  await expect(page.getByRole("textbox", { name: "快速 LLM Base URL" })).toHaveValue(
    "",
  );

  const mobileBase = page.getByTestId("mobile-backend-base");
  await mobileBase.fill("https://echodesk.yoliyoli.uk");
  await page.getByTestId("save-mobile-backend-base").click();
  savedMobileBase = await page.evaluate(() => window.localStorage.getItem("echodesk.mobileBackendBase") ?? "");
  expect(savedMobileBase).toBe("https://echodesk.yoliyoli.uk");

  await page.keyboard.press("Escape");
  await page.getByTestId("command-textarea").fill("帮我总结当前知识库");
  await expect(page.getByTestId("command-textarea")).toHaveValue("帮我总结当前知识库");
  const commandFontSize = await page
    .getByTestId("command-textarea")
    .evaluate((el) => window.getComputedStyle(el).fontSize);
  expect(commandFontSize).toBe("15px");

  const layout = await page.evaluate(() => {
    const pick = (selector: string) => {
      const el = document.querySelector(selector);
      if (!el) return null;
      const r = el.getBoundingClientRect();
      return { x: r.x, y: r.y, width: r.width, height: r.height };
    };
    const textarea = document.querySelector<HTMLElement>(
      "[data-testid='command-textarea'] textarea, textarea[data-testid='command-textarea'], [data-testid='command-textarea']",
    );
    return {
      documentWidth: document.documentElement.scrollWidth,
      viewportWidth: window.innerWidth,
      transcript: pick(".echodesk-transcript-pane"),
      output: pick(".echodesk-output-pane"),
      command: pick("[data-testid='command-bar']"),
      textarea: textarea ? textarea.getBoundingClientRect().toJSON() : null,
      attach: pick("[data-testid='command-attach-btn']"),
      send: pick("[data-testid='command-send-btn']"),
      placeholderFont: textarea
        ? window.getComputedStyle(textarea, "::placeholder").fontSize
        : "",
    };
  });

  expect(layout.documentWidth).toBeLessThanOrEqual(layout.viewportWidth + 1);
  expect(layout.transcript?.width).toBeGreaterThanOrEqual(560);
  expect(layout.output?.width).toBeLessThanOrEqual(460);
  expect(layout.placeholderFont).toBe("15px");
  expect(layout.attach?.height).toBe(40);
  expect(layout.send?.height).toBe(40);
  expect(Math.abs((layout.attach?.height ?? 0) - (layout.textarea?.height ?? 0))).toBeLessThanOrEqual(8);
  expect(Math.abs((layout.send?.height ?? 0) - (layout.textarea?.height ?? 0))).toBeLessThanOrEqual(8);
});
