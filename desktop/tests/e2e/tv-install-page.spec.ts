...
test("电视安装页：大屏链接和遥控器确认键路径可用", async ({ page }) => {
  ...
  await expect(apkLink).toHaveAttribute(
    "href",
    "https://github.com/yoligehude14753/echo-demo/releases/download/v0.2.3/EchoDesk-0.2.3-smart-tv.apk",
  );
  await apkLink.focus();
  ...
  await expect(bundleLink).toHaveAttribute(
    "href",
    "https://github.com/yoligehude14753/echo-demo/releases/download/v0.2.3/EchoDesk-0.2.3-smart-tv-oneclick.zip",
  );
