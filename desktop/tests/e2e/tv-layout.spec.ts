...
test("电视视口：横屏布局和遥控器确认键路径可用", async ({ page }) => {
  ...
  await page.route(/\/(api\/)?admin\/settings\/remote$/, async (route) => {
    ...
    await route.fulfill({
      ...
      body: JSON.stringify({
        ...
        fields: [
          { key: "llm_main_base_url", value: "https://model.example.com/v1", sensitive: false, source: "default" },
          { key: "llm_main_api_key", value: "", sensitive: true, source: "default" },
          { key: "llm_fast_base_url", value: "https://model.example.com/v1", sensitive: false, source: "default" },
  ...
  await expect(page.getByTestId("pill-backend")).toHaveAccessibleName("服务端", {
    timeout: 10_000,
  });
  ...
  // TV uses its server-issued owner scope and must hydrate that scope after resync/startup.
