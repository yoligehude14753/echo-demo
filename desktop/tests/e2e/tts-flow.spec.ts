...
test("TTS 未配置是可选中性态：不请求 speak、不弹错、不阻塞会议", async ({
  page,
}) => {
  await mockTtsDiag(
    page,
  ...
  await expect(toggle).toHaveAttribute("data-tts-state", "not_configured", {
    timeout: 10_000,
  });
  await expect(toggle).toContainText("播报未配置");
  await expect(page.getByTestId("meeting-status-bar")).toBeEnabled();
