...
test("采集计数器与入库计数器分别走（mock /capture/chunk 交替返回 ambient_stored）", async ({
  ...
  // Popover 使用用户可读的转写处理分布，不暴露底层门控术语。
  await cap.hover();
  await expect(
    page.locator(".ant-popover-inner").filter({ hasText: /本次转写处理结果/ }),
...
test("public/TV 模式直接显示并持久化 capture/chunk 返回的会议转写", async ({
  ...
  await expect(page.getByTestId("meeting-item-title")).toContainText("会议 ·");
  await expect(page.getByTestId("meeting-item-title")).not.toContainText(
    "m-local-",
  );
