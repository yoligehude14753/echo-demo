...
function canonicalRoles(config) {
  if (config.schemaVersion === 3) {
    if (!config.roles || typeof config.roles !== "object") {
      fail("invalid_schema", "schemaVersion 3 requires roles");
    }
    ...
    if (
      ...
      typeof config.runtimeEndpoint !== "object" ||
      config.runtimeEndpoint.environment !== "ECHODESK_BASE_URL"
    ) {
      fail("invalid_schema", "schemaVersion 3 requires runtimeEndpoint.environment=ECHODESK_BASE_URL");
    }
    if (!hub || typeof hub !== "object" || typeof hub.enabled !== "boolean") {
      fail(
        "invalid_paired_hub_role",
        "paired Hub sync gateway requires an explicit enabled flag",
      );
    }
    const hubBase = hub.enabled
      ? normalizeHttpBase(hub.baseUrl, {
        code: "invalid_paired_hub_role",
        label: "paired Hub sync gateway endpoint",
        requireHttps: true,
        allowPrivateHttp: false,
      })
      : null;
    if (hub.enabled && !hubBase) {
      fail(
        "invalid_paired_hub_role",
        "enabled paired Hub sync gateway requires an HTTPS endpoint",
      );
    }
    if (!hub.enabled && hub.baseUrl !== null) {
      fail(
        "invalid_paired_hub_role",
        "disabled paired Hub sync gateway must not define an endpoint",
      );
    }
    return {
      schemaVersion: 3,
...
function principalModeFor(runtimeMode, env) {
  ...
  if (runtimeMode === "release") {
    // Desktop capture is local-first. The bundled backend owns the fixed local
    // principal and starts capture without a cloud credential or Keychain
    // enrollment. Public mode remains an explicit operator opt-in; it is never
    // an implicit fallback for local data.
...
function localEndpointFor(env) {
  const injected = String(env.ECHODESK_BASE_URL ?? "").trim();
  if (!injected) {
    fail(
      "runtime_endpoint_missing",
      "ECHODESK_BASE_URL must be injected by the backend lifecycle owner",
    );
  }
  return normalizeLocalDevBase(injected);
}
...
function resolveBackendEndpoint(config, env = process.env, options = {}) {
  ...
  if (env.ECHO_PAIRED_HUB_SYNC_GATEWAY_BASE) {
    fail(
      "hub_sync_gateway_not_supported",
      "paired Hub sync gateway must come from packaged backend.config.json",
    );
  }
  ...
  const localBase = isPublic ? null : localEndpointFor(env);
  ...
  return Object.freeze({
    ...
    pairedHubSyncGatewayEndpoint: roles.hubBase,
